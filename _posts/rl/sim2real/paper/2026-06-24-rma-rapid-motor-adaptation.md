...
date: 2026-02-03 17:35:00 +0900
last_modified_at: 2026-02-03 17:35:00 +0900
...
앞선 논문들은 dynamics randomization으로 여러 물리 조건을 경험시키고, proprioceptive history에서 보이지 않는 접촉 상태를 추론했다. RMA는 여기서 한 단계 더 나아가 현재 환경에 맞춰 policy input 자체를 바꾼다.

> 마찰, payload, actuator 성능과 지형이 계속 바뀔 때, 하나의 robust policy가 모든 상황을 평균적으로 버티게 하는 것만으로 충분할까?

Kumar et al.의 **RMA: Rapid Motor Adaptation for Legged Robots**는 그렇지 않다고 본다.

같은 자세의 robot이라도 현재 조건이 다르면 필요한 action도 달라진다.
...
일반적인 domain-randomized policy가 환경 조건을 입력받지 않으면, 이 조건들을 하나의 policy mapping 안에서 평균내야 한다. 반면 RMA는 현재 환경을 나타내는 **extrinsics**를 policy input으로 넣는다.

문제는 실제 robot에서는 정확한 friction이나 payload를 바로 알 수 없다는 점.
...
Simulation에서는 mass, friction, motor strength와 terrain height를 알고 있으므로 좋은 context-conditioned policy를 먼저 학습한다. 그다음 최근 50 step, 즉 약 0.5초의 history만 보고 그 context를 추정하는 adaptation module을 따로 학습한다.
...
Training과 deployment의 data flow를 나누면 아래와 같다.
...
RMA는 physical parameter를 정확히 복원하는 classical system identification이 아니다. “Adaptation sample 0”도 sensor history를 쓰지 않는다는 뜻이 아니라 별도의 test-time optimization rollout이 없다는 의미. 따라서 latent component가 변해도 각 component를 friction이나 mass에 바로 대응시킬 수는 없다.
...
이 논문의 제목에서 **Rapid**는 단순히 network inference가 빠르다는 뜻이 아니다.

기존 adaptation 방법 중에는 새로운 환경에서 여러 rollout을 수집한 뒤 latent나 policy를 다시 최적화하는 방식이 있었다. 논문이 비교한 AWR 계열은 5~10초짜리 50 episode, 총 4~8분의 데이터를 사용한다.

RMA는 현재 한 trajectory의 최근 0.5초를 입력으로 사용하며, 저자들은 1초 미만의 적응을 목표로 한다.
...
실제 robot에서 adaptation을 위해 실패 rollout을 반복하지 않아도 된다는 점이 안전과 하드웨어 비용 측면에서 중요하다.
...
같은 $x_t$에서 friction이 달라도, payload가 달라도 policy input은 같다. Policy는 현재 조건을 명시적으로 구분하지 못한 채 여러 조건에서 평균적으로 괜찮은 action을 찾아야 한다.

이것이 반드시 실패한다는 뜻은 아니다. State history 자체에 환경 반응이 드러난다면 recurrent policy가 암묵적으로 적응할 수도 있다.

다만 RMA는 빠르게 변하는 robot state와 천천히 변하는 environment context를 구조적으로 분리한다.
...
$z_t$가 다르면 같은 robot state에서도 다른 action을 낼 수 있다.

### **2.2 RMA의 비교 대상은 단순한 nominal policy가 아니다**

논문의 **Robust** baseline도 training range 전체에서 dynamics randomization을 받는다. 차이는 latent $z_t$를 policy에 제공하지 않는다는 점.

따라서 RMA와 Robust의 비교는 다음 질문에 가깝다.

> 같은 randomization을 경험하더라도 현재 environment context를 추정해 조건부로 행동하는 것이 더 유리한가?

Simulation 결과에서 RMA의 success rate는 73.5%, Robust는 62.4%였다. 이 차이는 randomization을 없앤 효과가 아니라 **randomization에 online context inference를 추가한 효과**로 읽어야 한다.

### **2.3 평균적으로 안전한 policy가 항상 나쁜 것은 아니다**

논문은 context가 없는 robust policy가 보수적이 될 수 있다고 설명한다. 하지만 이것을 모든 domain-randomized policy에 대한 보편적 결론으로 확대하면 안 된다.
...
RMA가 보여준 것: 해당 A1 locomotion setup에서 explicit adaptation 구조가 robust baseline보다 좋은 결과를 냈다는 점.
...
Robot이 직접 관측하는 state를 $x_t$, 환경의 숨은 조건을 $e_t$라고 하겠다.

전체 dynamics는 대략 다음처럼 생각할 수 있다.
...
하지만 real deployment에서는 $e_t$를 직접 얻지 못한다.
...
이 변수들이 action에 대한 다음 state의 반응을 바꾼다.

따라서 단일 $x_t$만으로는 Markov state가 충분하지 않을 수 있다.

### **3.2 History는 environment를 드러내는 probe다**

Environment condition은 action과 response의 차이에서 드러난다.

Policy가 desired joint position을 보내면 environment에 따라 실제 joint velocity와 contact가 달라진다. 그 차이가 body roll/pitch와 foot-contact pattern에 남고, adaptation module은 최근 state-action sequence에서 hidden dynamics의 흔적을 추론한다.

RMA의 adaptation module은 다음을 계산한다.
...
논문에서는 $k=50$이며 policy가 100 Hz이므로 약 0.5초 이력이다.

Adaptation module은 **state history와 action history를 함께** 본다.

같은 state 변화라도 어떤 action을 가한 결과인지 알아야 environment response를 해석할 수 있기 때문이다.

### **3.3 식별 가능성에는 excitation이 필요하다**

History가 있다고 모든 parameter를 알 수 있는 것은 아니다.

Robot이 정지해 있으면 low friction과 high friction의 차이가 거의 드러나지 않을 수 있다. Payload도 충분한 가속이나 contact 변화가 있어야 response 차이가 나타난다.

즉 adaptation에는 다음 조건이 필요하다.
...
RMA는 별도의 active probing action을 설계하지 않는다. 걷기 동작 자체가 environment를 드러내는 excitation 역할을 한다.

따라서 “0.5초면 어떤 dynamics든 안다”가 아니라 다음처럼 이해해야 한다.
...
Privileged vector $e_t$는 아래 17개 요소로 구성된다.
...
Local terrain height는 네 발 아래 높이를 첫째 소수점까지 양자화한 뒤 그중 최대값을 사용한다.

즉 detailed height map이 아니라 매우 압축된 scalar. 저자들은 controller가 빠르고 정확한 local terrain sensing에 과도하게 의존하지 않도록 이 형태를 사용했다고 설명한다.
...
$\mu$는 hidden size 256, 128의 3-layer MLP이다.

여기서 8D latent는 단순한 차원 축소 결과가 아니다. $\mu$와 base policy가 PPO return을 기준으로 함께 학습되므로, $z_t$에는 **locomotion action을 바꾸는 데 유용한 정보**가 남도록 압력이 걸린다.

예를 들어 두 physical parameter 조합이 거의 같은 control response를 요구한다면, latent에서 서로 멀리 분리될 필요가 없다.
...
Policy는 hidden size 128의 3-layer MLP이며 12개 target joint angle을 출력한다.
...
이전 action을 넣는 것은 현재 observation만으로는 드러나지 않는 control history를 보완하고 action smoothness를 학습하는 데 도움을 준다.

### **4.4 Adaptation module $\phi$: temporal encoder**

Adaptation module은 각 state-action pair를 먼저 2-layer MLP로 32D embedding으로 바꾼다.

그 후 시간축에 다음 1D CNN을 적용한다.
...
Flattened CNN output은 linear projection을 거쳐 8D $\hat z_t$가 된다.

이 구조는 raw history 전체를 base policy가 매 control step 직접 처리하게 하지 않는다.
...
이 분리가 RMA의 계산 효율뿐 아니라 학습 문제의 역할 분리에도 중요하다.
...
Fixed-gain PD controller가 target을 torque로 변환한다.
...
즉 policy가 직접 motor torque를 출력하는 end-to-end torque policy는 아니다.

이 interface는 Sim2Real 관점에서 다음 역할을 한다.
...
반대로 고정 gain 자체가 맞지 않는 actuator나 큰 latency에서는 같은 policy 구조가 그대로 작동한다는 보장은 없다.

### **5.3 이 논문은 범용 velocity-command controller가 아니다**

Reward는 전방 속도를 최대 0.35 m/s까지 높이도록 설계되어 있다.
...
Policy input에 $(v_x^{cmd},v_y^{cmd},\omega_z^{cmd})$ 같은 command vector가 명시되어 있지 않는다.

따라서 이 결과를 “임의의 속도 명령을 추종하는 locomotion controller”로 해석하면 안 된다. 논문의 중심은 다양한 environment에서의 **forward locomotion adaptation**이다.
...
처음부터 history와 action을 한 network에 넣고 PPO로 모두 학습할 수도 있다.

하지만 그러면 policy는 동시에 두 문제를 풀어야 한다.
...
이 단계가 만든 것은 “환경 정보를 알 때의 좋은 controller”.

### **6.2 왜 physical parameter를 그대로 넣지 않고 encoder를 학습하는가**

$e_t$를 policy에 그대로 넣는 대신 8D latent로 압축하는 이유는 단순히 input을 줄이기 위해서만은 아니다.

Adaptation module의 target도 $e_t$가 아니라 $z_t$가 된다.
...
Physical parameter는 짧은 history에서 정확히 식별하기 어려울 수 있다. 반면 서로 다른 parameter 조합이 같은 행동 보정을 요구한다면 하나의 latent region으로 묶어도 locomotion에는 문제가 없다.

Simulation baseline에서 exact parameter $e_t$를 예측하는 SysID가 RMA보다 낮은 성능을 보인 결과는 이 설계를 지지한다.

다만 이것이 “물리 parameter 추정은 언제나 불필요하다”는 증명은 아니다. 이 task와 architecture에서 behavior-relevant latent가 더 잘 작동했다는 evidence이다.

### **6.3 PPO 설정**

논문과 supplementary의 Phase 1 설정을 표로 옮기면 아래와 같다.
...
Policy loss와 $0.5$배 value loss를 합쳐 학습하며, action standard deviation의 하한을 두어 exploration을 유지한다.

PPO를 사용했다는 사실도 중요하다.

RMA의 핵심 contribution: 새로운 RL optimizer가 아니라 아래 다섯 요소의 결합.
...
RMA는 reference trajectory와 predefined foot trajectory generator를 사용하지 않는다고 강조한다.

그렇다고 reward가 단순한 forward velocity 하나뿐인 것은 아니다.

### **7.1 열 개의 reward term**

Base frame의 linear velocity를 $\mathbf v$, angular velocity를 $\boldsymbol\omega$, roll/pitch를 $\boldsymbol\theta$, joint angle과 velocity를 $\mathbf q,\dot{\mathbf q}$라고 하겠다.
...
원문은 9번을 “Z Acceleration”이라고 부르지만 식은 $v_z$를 사용한다. 따라서 이 글에서는 식에 맞춰 vertical velocity penalty로 해석한다.

각 reward 묶음이 맡는 역할은 아래와 같다.
...
저자들은 work와 ground impact를 줄이는 bioenergetics-inspired constraint, uneven terrain training이 realistic gait transfer에 중요했다고 설명한다.

하지만 “reference motion이 없으므로 사람이 설계한 prior가 없다”는 뜻은 아니다.

다음 선택은 모두 강한 inductive bias이다.
...
즉 reference trajectory는 없지만, **어떤 motion을 좋은 motion으로 볼지는 reward와 control interface에 들어 있다**.

### **7.3 Penalty curriculum이 없으면 움직이지 않는 해가 쉽다**

학습 초기에 penalty 3~10을 모두 강하게 적용하면 action을 거의 내지 않는 것이 안전한 local optimum이 될 수 있다.

논문은 penalty multiplier를 다음처럼 증가시킨다.
...
$0<k_i<1$에서 1보다 작은 지수를 취하면 값이 점차 1에 가까워진다.

학습 순서: 먼저 움직임을 발견하고, 이후 work, impact, smoothness와 slip 조건을 점차 강화.
...
Mass, friction과 motor strength perturbation의 난이도도 학습 중 선형으로 늘린다.

반면 terrain 자체에는 curriculum을 적용하지 않고, 처음부터 고정 난이도의 fractal terrain profile을 무작위로 샘플링한다.
...
Main paper는 policy frequency 100 Hz와 simulation time step 0.025 s를 함께 보고한다. 두 수치가 단순히 같은 step을 뜻하면 일치하지 않지만, substep 또는 decimation 관계는 원문에 자세히 설명되어 있지 않는다.

따라서 재현할 때는 이를 임의로 40 Hz 또는 100 Hz로 정리하지 말고, 원 구현의 simulator integration과 control decimation을 확인해야 한다.
...
Testing은 training보다 범위가 넓고, episode 내부 parameter가 더 자주 바뀐다.

이 설계는 단순한 in-distribution test보다 강하다. RMA가 context를 한 번 추정한 뒤 episode 끝까지 고정하는 것이 아니라, 바뀐 dynamics를 계속 추적해야 하기 때문이다.

다만 test range가 training range의 연장선이라는 점도 기억해야 한다.

실제 세계의 모든 failure mode가 이 scalar range 안에 포함되는 것은 아니다.

### **8.3 무엇이 randomize되고 무엇이 latent target에 들어가는가**

Randomized parameter와 $e_t$의 component는 완전히 같은 목록이 아니다.

- $K_p$, $K_d$는 environment variation에는 포함된다.
- 17D $e_t$에는 payload 조건, motor strength, friction, local terrain height가 들어간다.
- Adaptation module은 PPO가 만든 8D $z_t$를 추정한다.

따라서 $z_t$를 “모든 randomization parameter의 정확한 압축본”이라고 부르는 것은 과하다.

Policy가 직접 받는 latent와 training 중 바뀌는 전체 simulator condition을 구분해야 한다.
...
Phase 1이 끝나면 base policy $\pi$와 environment encoder $\mu$를 freeze한다.

Simulation에서는 여전히 다음 oracle target을 계산할 수 있다.

$$
z_t=\mu(e_t)
$$

Adaptation module은 history로 이를 맞춘다.
...
여기서 $\phi$의 목적은 environment parameter 자체가 아니라, **이미 좋은 locomotion을 학습한 base policy가 사용하던 latent interface를 복원하는 것**.

### **9.2 Oracle trajectory만 사용하면 covariate shift가 생긴다**

가장 쉬운 데이터 수집 방식: true $z_t$로 안정적인 expert trajectory를 만든 뒤, 그 history와 $z_t$로 $\phi$를 학습.

하지만 deployment 초기의 $\hat z_t$는 틀릴 수 있다.

하지만 틀린 $\hat z$는 imperfect action을 만들고, body motion과 contact distribution을 바꾼다. Expert dataset에 없던 history가 생기면 $\phi$의 오차가 다시 커지는 feedback loop가 발생한다.

이것이 supervised imitation에서 흔한 covariate shift이다.

### **9.3 RMA는 predicted latent로 rollout한다**

RMA는 randomly initialized adaptation module의 $\hat z_t$를 실제 base policy에 넣어 trajectory를 수집한다.

1. Current $\phi$가 history에서 $\hat z_t$를 추정한다.
2. Frozen $\pi(x_t,a_{t-1},\hat z_t)$가 action을 내고 next state를 만든다.
3. Simulation oracle은 frozen $\mu(e_t)$로 target $z_t$를 계산한다.
4. 현재 $\phi$가 실제로 만든 history에서 $\lVert\hat z_t-z_t\rVert^2$를 최소화한다.

즉 $\phi$가 만든 오류 때문에 방문하게 된 state에서도 target $z_t$를 제공한다.

이 방식은 DAgger와 유사한 on-policy data aggregation 효과를 낸다.

“On-policy”라는 말이 여기서는 PPO policy를 다시 업데이트한다는 뜻이 아니다. Phase 2에서는 $\pi$와 $\mu$가 고정되고, 현재 $\phi$가 유도하는 trajectory distribution에서 $\phi$만 supervised learning으로 업데이트된다.
...
RMA가 real deployment에서 빠르게 적응할 수 있는 이유는 많은 학습 비용을 미리 simulation에 지불했기 때문이다.
...
Joint state와 contact는 빠르게 변한다. 안정적인 locomotion feedback은 100 Hz로 처리해야 한다.

반면 payload나 평균 friction 같은 context는 joint state보다 느리게 변한다. 50-step temporal convolution도 매 10 ms마다 계산할 필요가 없다.

이 분리는 low-cost onboard compute에서 다음 효과를 준다.
...
저자들은 state-action history를 하나의 policy가 직접 처리하는 대안도 실험했다고 설명한다.

저자들이 이 대안에서 관찰한 문제는 세 가지였다.
...
RMA architecture는 단순한 network modularity가 아니라 서로 다른 time scale을 가진 두 문제를 분리한 설계.
...
논문 setup에서는 문제가 되지 않았다고 보고하지만, 더 빠른 robot이나 다른 onboard stack에서는 timing을 별도로 검증해야 한다.
...
Learning baseline은 같은 architecture, reward와 hyperparameter를 사용했다고 논문은 설명한다.
...
각 수치는 세 개 random seed의 policy와 seed당 1,000 episode를 평균한 값. Test에서는 environment parameter를 step마다 확률 0.01로 다시 샘플링한다.
...
RMA와 Expert의 success 차이는 2.7%p이다.

$$
76.2-73.5=2.7
$$

즉 이 simulation setting에서는 0.5초 history 기반 $\hat z$가 oracle $z$와 상당히 가까운 control 성능을 냈다.

RMA와 Robust의 차이는 11.1%p.

$$
73.5-62.4=11.1
$$

Adaptation을 제거하면 52.1%로 더 크게 떨어진다.

큰 randomization range만으로는 Robust baseline과의 11.1%p 차이를 설명하기 어렵다. **현재 condition을 구분해 action을 바꾸는 mechanism**이 실제 이득을 만들었다.

### **11.4 `Adapt samples = 0`을 오해하지 말 것**

표의 RMA adaptation samples가 0인 이유는 test environment에서 별도의 optimization rollout을 수집하지 않기 때문이다.

그러나 RMA는 매 순간 최근 50 step의 sensor/action history를 사용한다.

정확한 뜻: **추가 test-time optimization sample이 0**. 관측 data 없이 adaptation한다는 뜻은 아니다.

### **11.5 Extreme simulation generalization**

Supplementary는 payload, terrain z-scale과 friction을 크게 변화시켜 추가 실험한다.
...
이 그림은 training range 밖으로 갈수록 모든 방법이 동일하게 유지된다는 뜻이 아니다. 변수별로 성능이 감소하며, RMA가 비교 방법보다 상대적으로 완만하게 저하된다는 결과.
...
실기에서 모든 simulation baseline을 돌리지 않은 이유는 실패가 robot damage로 이어질 수 있기 때문이다.
...
특히 uneven foam에서 adaptation 제거 model은 성공하지 못했고, 제조사 controller도 낮은 성공률을 보였다.

Rigid fractal terrain에서만 학습한 latent adaptation이 실제 deformable contact에서도 어느 정도 작동했다.

다만 deformable foam의 정확한 물리 model을 식별했다는 뜻은 아니다. Foam 때문에 나타난 body/contact response를 training 중 학습한 latent space의 유용한 영역으로 mapping했다고 보는 편이 정확하다.

StepDown 15 cm의 성공률은 논문 Figure 3의 내부 표기와 caption 문구가 서로 일치하지 않는다. Caption은 80%라고 설명하고 panel은 100%로 읽힌다.

그래서 **15 cm step-down을 반복적으로 성공했지만 정확한 비율 표기에는 원문 내부 불일치가 있다**고만 기록했다.

### **12.3 12 kg payload의 의미**

A1 자체 무게는 약 12 kg이다.

제조사 controller는 8 kg 부근부터 자세가 처지고 성능이 저하된다. Adaptation을 제거한 policy는 8 kg보다 큰 payload에서 잘 넘어지지는 않지만 앞으로 나아가지 못하는 경향을 보였다.

RMA는 최대 12 kg, 즉 robot body weight와 같은 payload를 높은 성공률로 운반했다고 논문은 보고한다.

여기서 성능은 단순한 fall avoidance가 아니다.
...
즉 robust하게 웅크리고 버티는 것과 task를 수행하며 적응하는 것을 구분해야 한다.
...
실험에서는 바닥 plastic sheet에 oil을 바르고 robot foot도 plastic으로 감쌌다.
...
저자들이 보고한 흐름은 normal gait $\rightarrow$ 약 2초에서 slip $\rightarrow$ gait/contact disturbance $\rightarrow$ $\hat z$ component 변화 $\rightarrow$ knee torque 증가 $\rightarrow$ gait period 회복.

RMA는 oily patch trial의 90%에서 성공했다.

여기서 중요한 해석: latent가 slip event에 반응했고, 그 latent를 조건으로 한 policy가 다른 action을 냈다는 점.

하지만 1번째 component가 “friction”, 5번째가 “slip”이라는 뜻은 아니다. Latent basis는 end-to-end로 학습되었으며 component 의미에 explicit supervision이 없다.

### **13.2 5 kg payload: disturbance 이후 더 큰 torque**

Supplementary는 주행 중 A1 등에 5 kg payload를 던지는 실험을 분석한다.
...
이것은 adaptation module이 static payload sensor를 읽은 결과가 아니다. Payload로 인해 생긴 state-action response를 history에서 읽은 결과.
...
Adaptation ablation과 simulation baseline을 함께 봐야 구조적 기여를 판단할 수 있다.
...
Outdoor trial에서 보고된 결과는 아래와 같다.
...
이 terrain은 simulation의 fractal height field와 물리적으로 동일하지 않는다.
...
RMA의 의미는 이 현상을 정확히 simulation한 것이 아니라, 다양한 perturbation에서 학습한 conditional behavior와 online latent inference가 실제 response 변화에도 유효했다는 데 있다.

### **14.1 무엇을 강하게 말할 수 있는가**

- Simulation-only training 뒤 A1에 직접 배포되었다.
- Reference trajectory와 predefined foot trajectory generator 없이 gait를 학습했다.
- 동일한 architecture가 다양한 indoor/outdoor 조건에서 동작했다.
- Adaptation 제거 ablation보다 실제 task 성능이 좋았다.
- Slip과 payload event 뒤 latent와 torque/gait가 함께 변했다.
...
Outdoor 결과는 강한 deployment evidence지만, terrain별 trial count와 조건이 완전히 통제된 대규모 benchmark는 아니다.
...
Classical system identification은 관측 history $H_t$에서 physical parameter를 추정한다.
...
모델 기반 controller가 이 parameter를 사용하려면 물리적 정확성과 단위가 중요하다.
...
$z_t$는 PPO return을 높이도록 policy와 함께 형성된 latent이다.
...
논문의 SysID success rate는 56.5%, RMA는 73.5%.

RMA가 direct SysID보다 높았던 이유는 세 가지로 생각해볼 수 있다.
...
예를 들어 서로 다른 mass-friction 조합이 비슷한 torque와 gait adjustment를 요구한다면, policy latent에서는 비슷하게 표현해도 된다.

다만 direct SysID baseline의 특정 network와 학습 setup에서 나온 결과이므로, 더 좋은 system identification 방법 전체가 열등하다고 결론 내리면 안 된다.
...
주의할 점: environment encoder도 PPO gradient를 받는다.

$\mu$를 미리 만든 arbitrary autoencoder로 보면 안 된다. Latent는 reconstruction이 아니라 control return에 맞춰진다.
...
실제 구현에서는 gradient boundary를 명확히 해야 한다.
...
여기서 production-grade 구현이라면 다음을 추가해야 한다.
...
논문 architecture를 복사하는 것과 안전하게 real robot에 배포하는 것은 별개의 작업이다.
...
이 비교는 adaptation module 자체의 기여를 본다.

$$
\text{Success: }73.5\%\;\text{vs}\;52.1\%
$$

Base policy와 latent-conditioned training만 있고 runtime latent inference가 없으면 성능이 크게 떨어진다.

### **17.2 RMA vs Robust**

이 비교는 environment-conditioned control의 가치를 본다.

$$
\text{Success: }73.5\%\;\text{vs}\;62.4\%
$$

둘 다 randomization을 보지만 RMA는 현재 context를 추정해 조건부 action을 낸다.

### **17.3 RMA vs Expert**

이 비교는 adaptation module이 oracle latent에 얼마나 가까운 control 결과를 내는지 본다.
...
AWR은 40k adaptation sample을 사용했지만 success가 41.7%였다.

Test dynamics가 episode 중 계속 바뀌는 setting에서는 여러 rollout을 모아 느리게 latent를 최적화하는 방식이 현재 environment를 따라가기 어렵다.

여러 rollout을 요구하지 않고 현재 history에서 바로 context를 갱신한다는 점에서 `rapid`의 의미가 가장 직접적으로 드러나는 비교이다.
...
Adaptation module은 simulation에서 생성된 history와 latent mapping을 학습한다.
...
$\hat z_t$는 의미 없는 extrapolation이 될 수 있다.
...
짧은 history에서 서로 다른 environment가 같은 response를 만들면 구분할 수 없다.
...
이면 $\phi$가 둘을 식별하기 어렵다.

Policy 관점에서 같은 행동이 필요하다면 문제가 없지만, 나중에 다른 action에서 response가 갈라지면 잘못된 latent가 위험할 수 있다.

### **18.3 Contact 이후에야 알 수 있다**

RMA는 proprioceptive adaptation.
...
따라서 exteroceptive perception과 planning을 대체하지 않는다.

### **18.4 Latent interpretability**

$z$는 control return에 맞춰 학습되므로 각 축의 물리 의미가 보장되지 않는다.

Plot에서 component 1과 5가 slip 뒤 변했다고 해서 $z_1$을 friction coefficient, $z_5$를 slip probability라고 이름 붙이면 안 된다.

Latent intervention, disentanglement test 또는 decoder 검증이 추가로 필요하다.

### **18.5 실제 실험의 표본 크기**

Indoor experiment는 보통 method당 5 trial이고, 위험한 실패는 2회 후 중단했다.

이는 hardware safety를 고려하면 합리적이지만, 작은 차이를 통계적으로 확정하기에는 제한적이다.

Outdoor result도 매우 인상적인 deployment evidence지만 표준화된 대규모 benchmark는 아니다.

### **18.6 Command와 task 범위**

이 논문은 forward locomotion을 중심으로 한다.

Velocity command tracking, turning, recovery, manipulation과 navigation까지 같은 latent가 일반화된다는 것을 보여주지 않는다.

RMA architecture는 범용적이지만 이 논문의 evidence 범위는 A1 forward locomotion.

### **18.7 Real-time safety layer가 논문의 중심은 아니다**

Joint target clipping, torque saturation, watchdog와 fallback controller 같은 production safety mechanism은 architecture 설명의 중심이 아니다.

실제 robot 적용에서는 adaptation accuracy와 별개로 다음이 필요하다.
...
앞선 Challenging Terrain 논문과 RMA는 history를 쓰지만 목적과 구조가 다르다.
...
또한 RMA는 predefined foot trajectory generator를 사용하지 않지만, Lee et al.은 PMTG와 analytic IK를 사용했다.

어느 쪽이 절대적으로 우월하다는 의미가 아니다. 두 논문은 서로 다른 inductive bias와 deployment 목표를 선택했다.
...
여러 환경에서 하나의 평균적인 action을 내는 것과, 현재 환경을 추정해 action을 바꾸는 것은 다른 문제이다.

### **원칙 2: 정확한 물리 parameter보다 task-relevant latent가 유리할 수 있다**

Controller가 필요로 하는 equivalence class만 표현하면 짧은 history에서도 추정 문제가 쉬워질 수 있다.

### **원칙 3: Privileged learning은 deployability를 함께 설계해야 한다**

Simulation에서만 알 수 있는 $e_t$를 넣는 것으로 끝나지 않고, real history로 복원 가능한 $z_t$ interface를 만들어야 한다.

### **원칙 4: Adaptation model은 자기 오류가 만든 trajectory에서 학습해야 한다**

Oracle trajectory만 학습하면 deployment covariate shift에 약하다.

### **원칙 5: Fast control과 slow context를 서로 다른 rate로 운영할 수 있다**

모든 network를 최고 주기로 돌리지 않아도 된다. 다만 stale estimate와 timestamp alignment를 관리해야 한다.

### **원칙 6: 그림 속 latent 변화와 physical semantics를 혼동하지 않는다**

Latent는 행동에 유용한 representation이지 자동으로 해석 가능한 system parameter가 아니다.
...
RMA를 “history를 넣은 PPO”라고만 보면 구조가 흐려진다. 먼저 privileged PPO로 환경을 알 때 잘 걷는 policy를 만들고, policy가 필요로 하는 environment 정보를 8D behavior-relevant latent로 압축한다. 실제 robot에서는 최근 50-step state-action history로 그 latent를 추정한다.

세 network의 data flow는 아래와 같다.
...
Phase 1은 PPO로 1.2 billion simulation step을 사용하고, Phase 2는 predicted latent가 만든 trajectory에서 supervised MSE로 adaptation module을 학습한다. Deployment에서는 base policy 100 Hz, adaptation module 10 Hz와 fixed-gain PD controller가 비동기로 동작한다.

Simulation에서 RMA는 73.5% success로 Robust 62.4%, direct SysID 56.5%, adaptation 제거 52.1%를 앞섰고 oracle latent를 쓰는 Expert 76.2%에 근접했다. 실제 A1에서는 payload, foam, mattress, incline, oil, sand, mud, vegetation, stairs와 debris를 평가했다.
...
다음 편의 **Learning to Walk in Minutes Using Massively Parallel Deep Reinforcement Learning**은 이런 locomotion policy 학습의 wall-clock time을 GPU 병렬 simulation으로 줄인다.
