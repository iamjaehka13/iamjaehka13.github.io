...
논문의 해법은 simulator를 두 부분으로 나누는 것입니다.

1. Link, inertia, contact처럼 물리식으로 잘 기술되는 부분은 **rigid-body simulator**가 계산합니다.
2. Command에서 torque까지의 복잡한 부분은 실제 robot data로 학습한 **actuator network**가 계산합니다.
...
여기서 neural network가 전체 physics를 대체한 것은 아닙니다. Rigid body와 contact는 analytical simulator에 남기고 command-to-torque subsystem만 actuator net이 담당합니다. 또한 4분 미만의 real actuator data가 simulator identification에 들어갔으므로 real data가 전혀 없는 설정도 아닙니다. Policy의 trial-and-error만 simulation에서 수행됐습니다.

Actuator model의 가치는 torque prediction graph 하나가 아니라 real-robot ablation에서 드러납니다. 같은 pipeline에서 model만 바꿨을 때 transfer 성공 여부가 갈렸습니다. Locomotion, high speed와 recovery도 하나의 universal controller가 아니라 pipeline을 공유하는 별도 policy입니다.
...
하지만 이 논문에서 policy action $a_t$는 torque가 아니라 desired joint position $q_t^{\mathrm{des}}$입니다. 실제 transition은 적어도 두 단계로 나뉩니다.
...
Rigid-body model을 아무리 잘 만들어도 $G$가 틀리면 같은 action에서 다른 torque가 나옵니다. 그러면 foot-contact timing과 impulse가 바뀌고, 그 차이가 다음 state와 다음 action으로 누적됩니다.

이 때문에 actuator error는 부차적인 torque 오차가 아니라 **policy가 학습하는 transition kernel의 오차**입니다.
...
실제 robot에서는 진짜 actuator와 low-level controller가 그 역할을 합니다.

### **2.3 빠른 simulator도 방법론의 일부다**

논문에 보고된 계산량은 아래와 같습니다.
...
Hybrid simulator 실행 시간의 약 절반은 actuator nets 평가에 사용되었습니다. 즉 learned model은 공짜가 아니지만, 수억 개 transition을 생성할 수 있을 만큼 작고 빠르게 설계되었습니다.
...
ANYmal의 link geometry와 inertial property는 CAD model에서 가져옵니다. Contact는 Coulomb friction cone을 존중하는 hard-contact solver로 계산합니다.

하지만 CAD가 곧 실제 robot은 아닙니다.

- Cable과 electronics의 mass가 완전히 반영되지 않을 수 있습니다.
- 조립 오차로 center of mass와 joint 위치가 달라질 수 있습니다.
- Robot configuration과 payload가 바뀔 수 있습니다.

저자들은 inertial property에 최대 약 20%의 오차가 있을 수 있다고 봤습니다.

### **3.2 하나의 ANYmal이 아니라 30개 model에서 학습한다**

Policy는 30개의 stochastic ANYmal model에서 학습됩니다.
...
여기서 $f_\phi$는 actuator net입니다.

이 구조에서 두 도구의 역할은 다릅니다.
...
즉 randomization으로 모든 것을 덮는 것이 아닙니다. **평균 model을 먼저 현실 쪽으로 옮기고, 그 주변의 uncertainty만 distribution으로 만듭니다.**
...
ANYmal SEA는 motor, high-ratio transmission, elastic element와 encoder 두 개로 구성됩니다.

Command-to-torque chain 안에는 다음 hidden state가 존재합니다.
...
저자들이 인용한 기존 analytical SEA model은 거의 100개 parameter를 요구했습니다. 일부 parameter는 측정할 수 있지만, 일부는 datasheet를 믿거나 실험적으로 맞춰야 합니다.

특히 limited bandwidth가 있는 SEA에서는 response delay가 amplitude에 따라 달라질 수 있습니다. 고정 latency 하나를 넣는 것으로는 충분하지 않습니다.

### **4.2 입력은 현재값 하나가 아니라 짧은 history다**

Position error를 다음처럼 정의하겠습니다.

$$
e_t=q_t^{\mathrm{des}}-q_t
$$

Actuator net은 joint별로 다음 값을 받습니다.
...
현재와 10 ms, 20 ms 전 state를 함께 쓰는 이유는 actuator가 partially observable하기 때문입니다.
...
실제 torque는 다를 수 있습니다. 짧은 history가 hidden controller state와 response delay의 흔적을 제공합니다.

History가 길수록 무조건 좋은 것은 아닙니다.

- 너무 짧거나 sparse하면 high-frequency dynamics를 놓칩니다.
- 너무 길거나 dense하면 overfitting과 계산량이 증가합니다.
- 최소한 communication delay와 mechanical response time보다 긴 window가 필요합니다.

논문은 validation error를 보며 이 구성을 경험적으로 정했습니다.

### **4.3 4분 미만의 real data를 어떻게 모았는가**

Actuator net은 supervised regression으로 학습됩니다.
...
Data collection controller는 sine-wave foot trajectory를 만들고 inverse kinematics로 joint target을 계산했습니다.
...
12개 actuator에서 동시에 data가 들어오기 때문에 짧은 시간에도 sample 수가 많습니다. 저자들은 사람이 robot을 밀어 disturbance도 추가했습니다.

Sample 개수보다 **excitation coverage**를 봐야 합니다. 낮은 frequency만 수집하면 locomotion policy가 만드는 빠른 command에서 model이 부정확해지고, simulation 안에서 비현실적인 oscillation이 생겼습니다.
...
두 activation의 validation RMS는 비슷한 0.7-0.8 Nm였고, 저자들은 더 빠른 softsign을 선택했습니다.
...
Test error가 validation error보다 커졌다는 점도 중요합니다. Data-collection controller와 learned locomotion policy가 만드는 state distribution이 완전히 같지 않기 때문입니다.

그럼에도 learned model의 test error는 ideal model보다 훨씬 작았습니다.

이 결과가 보여주는 것은 **torque prediction fidelity**입니다. 이것만으로 policy transfer를 증명하는 것은 아닙니다. Transfer 근거는 다음 actuator-model ablation에서 나옵니다.

### **4.6 결정적 ablation: torque graph가 아니라 실제 한 걸음**

저자들은 policy training 절차는 동일하게 두고 actuator model만 세 종류로 바꿨습니다.
...
두 alternative policy에서는 limb가 격하게 떨렸습니다. 저자들은 multiple delay와 limited bandwidth를 충분히 설명하지 못한 것이 원인일 가능성이 높다고 해석했습니다.

Analytical model은 대충 만든 baseline도 아닙니다. 실제 controller code와 identified parameter를 넣고, latency·damping·friction을 real data에 맞춰 **일주일 이상** 조정했지만 성공하지 못했습니다.
...
다만 이 ablation을 일반 법칙으로 확대하면 안 됩니다.
...
이 논문은 PPO나 SAC가 아니라 **Trust Region Policy Optimization, TRPO**를 사용합니다.

목적은 일반적인 discounted return maximization입니다.
...
TRPO는 update 전후 policy가 지나치게 멀어지지 않도록 KL divergence constraint를 둡니다.
...
저자들은 모든 task에 원 TRPO의 default parameter를 사용했다고 보고합니다. 약 2.5억 transition을 4시간가량에 생성·처리할 수 있었던 것은 algorithm 자체보다 fast simulator와 custom implementation의 조합 덕분입니다.

### **5.2 Observation은 real robot에서 얻을 수 있는 값만 사용한다**

Policy observation은 다음 요소를 포함합니다.
...
Fall recovery에서는 robot이 발로 서 있지 않기 때문에 flat-ground kinematics 기반 base-height estimate를 신뢰할 수 없습니다. 따라서 recovery policy observation에서는 height를 제거했습니다.

IMU orientation도 quaternion 전체를 그대로 쓰지 않습니다. IMU로 안정적으로 관측 가능한 roll/pitch에 해당하는 gravity direction $\phi^g\in S^2$를 사용합니다. Global yaw는 task에 필요하지 않고 drift가 생길 수 있습니다.

Joint history는 locomotion 학습에 필수였습니다. 저자들은 history가 명시적 foot-force sensor 없이 contact를 간접 추론하게 해준 것으로 해석합니다. 이것은 실험적 해석이지, contact state가 network 내부에 직접 복원됐음을 증명한 것은 아닙니다.

### **5.3 Noise와 normalization은 선택 사항이 아니었다**

Real joint velocity는 position을 수치 미분해 얻으므로 noise가 큽니다. Training에서는 다음 noise를 추가했습니다.
...
원문의 base angular velocity noise unit 표기는 `m/s`로 되어 있지만, quantity 자체는 angular velocity이므로 단위를 그대로 확대 해석하지 않는 편이 안전합니다.

저자들은 noisy velocity를 빼고 position history만으로 velocity를 추론하게 했을 때 학습 자체가 실패했다고 보고했습니다. 이론적으로 finite difference가 가능해도, 실제 non-convex optimization에서는 유용한 preprocessing과 feature 제공이 중요하다는 예입니다.

### **5.4 Action은 low-impedance joint position target이다**

Policy는 12개 desired joint position을 출력합니다.

$$
a_t=q_t^{\mathrm{des}}
$$

Low-level command는 fixed-gain impedance form입니다.
...
Low-level impedance controller의 설정은 아래와 같습니다.
...
이것은 미리 만든 time-indexed joint trajectory를 강하게 추종하는 전통적 position controller와 다릅니다. Policy가 매 state에서 새로운 target을 만들기 때문에 position error가 생길 것을 학습 중부터 고려합니다.

저자들은 torque action보다 position action이 초기 exploration에서 서 있는 controller에 가깝게 시작하고, action landscape가 더 smooth해 학습하기 쉬웠다고 설명합니다.
...
저자들의 해석은 out-of-distribution state에서 unbounded ReLU feature가 지나치게 큰 action을 만들 수 있다는 것입니다. Bounded tanh는 disturbance를 받았을 때 더 보수적인 output을 만듭니다.

이 결과 역시 “tanh가 항상 최고”라는 일반 법칙보다 다음 설계 원칙으로 읽는 편이 좋습니다.
...
Locomotion reward에는 velocity tracking뿐 아니라 torque, joint speed, foot clearance, slip, orientation과 smoothness cost가 들어갑니다.

모든 constraint penalty를 처음부터 크게 주면 “움직이지 않기”가 쉬운 local optimum이 될 수 있습니다. 반대로 penalty가 너무 약하면 빠르지만 거칠고 위험한 motion을 배웁니다.

논문은 objective term을 먼저 배우고, 이후 constraint를 강화하는 curriculum을 사용합니다.
...
논문의 supplementary material은 velocity error에 bounded logistic kernel을 사용합니다.
...
초기 error가 커도 cost가 무한히 커지지 않도록 해, 넘어져 episode를 끝내는 것이 오히려 유리해지는 상황을 줄입니다.

Locomotion cost는 command tracking과 actuator·contact regularization으로 나뉩니다.
...
Gait sequence나 contact schedule은 reward에 직접 주어지지 않았습니다. Learned controller가 speed에 따라 walking trot과 flying-trot-like pattern을 선택했습니다.

### **6.3 Recovery는 initial-state distribution이 핵심이다**

Fall recovery는 upright locomotion과 초기 state가 완전히 다릅니다.
...
Recovery에서는 41개 collision body가 관여하고, body collision geometry의 크기와 위치도 randomize했습니다. 비현실적인 internal collision이 생기는 sample은 제거했습니다.

Recovery cost는 upright orientation뿐 아니라 다음 safety-related term을 포함합니다.
...
“뒤집기만 하면 된다”가 아니라 impact와 fragile component collision을 피하도록 reward를 다듬는 데 약 일주일이 걸렸습니다. 이는 reward engineering이 사라진 것이 아니라 simulation 안으로 이동했음을 보여줍니다.
...
Locomotion trajectory는 최대 6초이며, joint limit violation이나 base-ground collision 시 종료됩니다.

Discount를 너무 낮추면 immediate movement는 배우지만 natural standing posture가 약해졌고, 너무 높으면 convergence가 느려질 수 있었습니다.
...
Policy는 command sequence나 command 변경 timing을 미리 알지 못합니다. Runtime에 들어오는 command를 observation으로 받아 gait를 바꿉니다.
...
30초 동안 2초마다 command를 바꿔 총 15번의 transition을 만들었습니다.
...
학습 범위의 forward command는 최대 1.0 m/s였지만, 다른 command를 0으로 두고 1.23 m/s를 명령했을 때 실제 1.2 m/s를 안정적으로 달성했습니다. 이는 제한된 범위 밖의 아주 가까운 command에 대한 결과이지, arbitrary extrapolation을 의미하지는 않습니다.
...
논문은 같은 command profile에서 prior controller의 linear error가 약 95%, yaw-rate error가 약 60% 더 높았다고 보고합니다.
...
0.25, 0.5, 0.75, 1.0 m/s command를 각각 4.5초씩 step input으로 주었습니다.
...
저자들은 learned policy가 knee를 10-15도 더 곧게 펴는 posture를 사용해 torque를 줄였다고 분석합니다. 기존 controller에서는 이 posture가 fall rate를 높여 쓰기 어려웠습니다.

여기서 “RL이 항상 model-based보다 낫다”고 결론 내리면 과합니다. 비교 대상은 같은 ANYmal에서 사용하던 특정 controller와 gait 설정입니다. 하지만 동일 hardware에서 accuracy, power, torque를 함께 측정했다는 점은 강한 실기체 evidence입니다.

---

## **8. Result 2: Hardware Limit에 가까운 High-Speed Locomotion**

High-speed policy의 command range는 forward direction에 더 집중되어 있습니다.
...
Real test에서는 command를 서서히 1.6 m/s까지 높이고, 10 m를 달린 뒤 0으로 낮췄습니다.
...
Speed는 최소 3 gait cycle에 걸쳐 평균했습니다.

Gait는 ordinary flying trot과 비슷하지만 flight phase가 더 길고 좌우 duration이 비대칭입니다. 저자들은 하나의 near-optimal mode일 수 있다고 해석했으며, 자연 동물 gait와 같다고 주장하지 않았습니다.

이 결과의 의미는 단순 최고 속도보다 **constraint-aware behavior가 policy training 안에서 나왔다는 점**입니다.

전통적인 modular controller에서는 planner가 뒤쪽 actuator limit을 완전히 알지 못해 실행 불가능한 reference를 만들 수 있습니다. 이 policy는 training simulation에 torque와 joint-speed constraint가 포함되어 있어, limit에 닿으면서도 실행 가능한 motion을 직접 학습했습니다.
...
Recovery는 locomotion보다 contact topology가 훨씬 복잡합니다.

- Foot뿐 아니라 knee, side panel, body가 바닥에 닿을 수 있습니다.
- 자기 다리와 body 사이 internal contact도 생깁니다.
- 미리 정한 contact sequence가 없습니다.
- Momentum을 만들어 몸 전체를 굴려야 합니다.

Real experiment에서는 upside-down에 가까운 pose와 자기 다리 위에 몸이 얹힌 pose를 포함해 **9개의 random configuration**을 시험했습니다.
...
아홉 configuration의 결과를 묶으면 아래와 같습니다.
...
“첫 시도 성공”과 “최종 100%”를 혼동하면 안 됩니다. 처음부터 모든 random state에서 100%였다는 뜻이 아니라, 첫 hardware deployment가 성공했고 이후 joint-velocity constraint를 조정해 success rate를 높였다는 설명입니다.

또한 recovery policy는 simulation에서 20 Hz로 학습했지만 real test에서는 100 Hz로 실행해도 성능이 비슷했습니다. 저자들은 flip-up motion의 joint speed가 대부분 6 rad/s 이하였기 때문에 가능했다고 설명합니다. Locomotion처럼 빠른 behavior에는 200 Hz가 필요했습니다.
...
Recovery network는 100 Hz에서도 single CPU core 계산량의 약 0.25%만 사용했습니다. 학습 때 복잡한 simulator를 사용해도 deployment에는 small MLP만 남습니다.

### **10.2 Real actuator data는 simulator용이고 policy는 simulation-only다**

이 논문을 “real data를 전혀 쓰지 않은 Sim2Real”이라고 부르면 틀립니다.

Real actuator data와 simulation-only policy training의 관계는 아래 순서로 구분해야 합니다.
...
저자들은 모든 policy를 수정 없이 real robot에서 3개월 이상 시험했습니다.

그 사이에 약 2 kg의 configuration 차이와 기존보다 spring이 3배 stiff한 새 drive를 포함한 hardware change가 있었지만 policy가 계속 동작했다고 보고합니다.

이 결과는 robustness의 유용한 evidence지만, systematic sweep이나 confidence interval이 있는 benchmark는 아닙니다. Discussion의 field observation으로 해석하는 것이 적절합니다.
...
Dynamics randomization은 simulator parameter $\theta$를 distribution으로 만듭니다.

$$
\theta\sim p(\theta)
$$

장점은 exact identification에 덜 의존한다는 것입니다. 하지만 nominal actuator response 자체가 크게 틀린 경우 아주 넓은 distribution이 필요하고, policy가 지나치게 보수적이 될 수 있습니다.

### **11.2 Tan et al.: analytical actuator model을 정교하게 만든다**

Minitaur에서는 motor voltage, back-EMF와 latency를 반영한 compact analytical actuator model이 효과적이었습니다.

### **11.3 Hwangbo et al.: 어려운 subsystem만 학습한다**

ANYmal SEA에서는 actuator/software stack이 더 복잡했습니다. 그래서 command-to-torque mapping을 learned model로 대체합니다.
...
1. ANYmal SEA에서는 actuator-model fidelity가 locomotion transfer 성공 여부를 바꿨습니다.
2. History-conditioned actuator network는 ideal model보다 real torque를 훨씬 정확히 예측했습니다.
3. Physics와 learned subsystem model을 결합해 약 500K steps/s의 빠른 hybrid simulator를 만들 수 있었습니다.
4. Simulation-trained policy가 command tracking, 1.5 m/s running, 3초 이내 recovery를 real ANYmal에서 수행했습니다.
5. 같은 hardware의 기존 controller와 비교해 tracking error, torque와 power가 개선되었습니다.

### **12.2 조심해서 말해야 하는 것**

1. **모든 analytical actuator model이 실패한다:** 이 논문은 ANYmal의 특정 SEA와 구현에 대한 결과입니다.
2. **Real data가 전혀 필요 없다:** Actuator net 학습에는 real data와 torque measurement가 필요합니다.
3. **하나의 universal policy다:** 세 task는 별도 reward와 별도 policy로 학습했습니다.
4. **Reward engineering이 사라졌다:** Recovery cost와 safety term을 다듬는 데 약 일주일이 걸렸습니다.
5. **모든 terrain에서 robust하다:** 이 논문의 중심은 flat-ground agile skill과 recovery이며 exteroceptive rough-terrain locomotion이 아닙니다.
6. **Actuator model만 맞으면 transfer된다:** Rigid-body randomization, observation noise, bounded policy, history와 curriculum도 함께 사용했습니다.
...
Actuator network를 만들려면 command, position, velocity와 torque를 정확한 timestamp로 기록해야 합니다.

Robot에 torque sensing이나 logging infrastructure가 없으면 같은 접근을 바로 적용하기 어렵습니다. Torque label 자체가 biased하면 network는 그 bias를 학습합니다.

### **13.2 Training coverage 밖에서는 model도 틀릴 수 있다**

Data collection은 1-25 Hz, 5-10 cm foot trajectory를 사용했습니다. 실제 policy가 더 높은 frequency, 더 큰 error나 다른 thermal condition을 만들면 actuator net은 extrapolation합니다.

Validation split이 random sample split이라면 같은 excitation trajectory의 인접 sample이 train과 validation에 함께 들어갈 가능성도 있습니다. 논문은 trajectory-level holdout 여부를 상세히 보고하지 않습니다.

현대적으로 재현한다면 최소한 다음 holdout을 분리하는 편이 좋습니다.
...
논문은 12개 actuator dynamics가 서로 독립이라고 가정합니다. Shared power supply, hydraulic accumulator나 thermal coupling이 강한 robot에서는 joint별 network로 충분하지 않을 수 있습니다.

그 경우 actuator model 입력에 bus voltage, temperature, neighboring-joint command를 넣거나 multi-actuator model을 고려해야 합니다.

### **13.4 Contact와 state-estimation gap은 남는다**

Actuator fidelity가 좋아도 ground compliance, friction transition, impact loss와 base velocity estimator의 bias는 완전히 해결되지 않습니다.

이 논문은 flat terrain 중심이고, terrain perception은 다루지 않습니다. 다음 rough-terrain 논문에서는 proprioception과 terrain curriculum, 이후 연구에서는 exteroception과 adaptation이 중요해집니다.

### **13.5 Safety cost도 simulator fidelity에 의존한다**

Recovery policy가 simulation에서 body-impact cost를 줄였다고 해서 real hardware stress가 자동으로 안전한 것은 아닙니다.
...
같은 요소는 별도 hardware safety 검증이 필요합니다.

---

## **14. 재현한다면 무엇을 먼저 확인할까**

이 논문의 핵심을 다른 robot에서 재현할 때는 RL부터 돌리기보다 다음 순서가 안전합니다.
...
Ideal PD baseline과 가능한 analytical actuator model을 먼저 구현합니다. Learned model의 이득을 판단할 비교 대상이 필요합니다.

### **Step 3. Excitation coverage를 설계한다**

Policy가 사용할 position error, velocity와 frequency 범위를 안전하게 덮습니다. Random command만 넣기보다 frequency sweep과 amplitude sweep을 분리합니다.

### **Step 4. Actuator model을 out-of-distribution에서 평가한다**

RMS 하나만 보지 말고 다음을 확인합니다.
...
Joint-position bound, torque limit, velocity limit, emergency stop과 fall-protection 환경을 둡니다. 논문의 1.5 m/s나 dynamic recovery를 첫 실험으로 따라 하면 안 됩니다.

---

## **15. Actuator Model은 Simulator의 부품이다**

Hwangbo et al.은 rigid body와 contact는 physics로 계산하고, 식별하기 어려운 SEA command-to-torque mapping만 history-conditioned actuator net으로 바꿨습니다. 4분 미만의 400 Hz actuator data에서 100만 개 이상의 sample을 얻었고, policy trial-and-error는 이 hybrid simulator 안에서 수행했습니다.

Learned model의 torque RMS는 validation 0.740 Nm, policy test 0.966 Nm였습니다. 더 직접적인 근거는 ideal·analytical actuator model policy가 실제 로봇에서 걷지 못한 반면 learned model policy는 transfer됐다는 ablation입니다. 이 결과는 actuator net 하나만의 효과로 분리할 수는 없습니다. Stochastic rigid-body model, observation noise, bounded policy, state history와 cost curriculum이 함께 사용됐습니다.

이 pipeline에서 학습한 별도 TRPO policy들은 실제 ANYmal에서 command tracking, 1.5 m/s running과 3초 이내 fall recovery를 수행했습니다.
...
다음 편에서는 이 actuator-aware pipeline이 uneven terrain으로 확장되면서 terrain curriculum과 observation history를 만나게 됩니다.
