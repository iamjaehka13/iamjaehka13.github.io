...
앞선 Hwangbo et al.의 논문은 learned actuator network와 stochastic rigid-body model을 결합해 flat ground locomotion, high-speed running과 fall recovery를 실제 ANYmal로 옮겼다.

Lee et al.의 **Learning Quadrupedal Locomotion over Challenging Terrain**은 그 pipeline을 자연 지형으로 확장한다.

문제는 simulator를 더 정교하게 만드는 것만으로 해결되지 않는다.
...
이 모든 deformable contact와 obstruction을 빠른 rigid-body simulator에 사실적으로 넣기는 어렵다.
...
논문의 구성은 teacher, student와 curriculum 세 축으로 나뉜다.
...
Deploy되는 student는 camera, LiDAR, foot-contact sensor와 terrain height map을 사용하지 않는다. Joint encoder, IMU와 state estimator가 주는 proprioceptive stream만 사용한다.

다만 이것을 “눈 없이 terrain을 정확히 안다”고 표현하면 과하다.

> Student는 terrain map을 명시적으로 복원해 planning하는 것이 아니라, 과거 body response에서 control에 필요한 hidden condition의 흔적을 암묵적으로 인코딩한다.

여기서 `blind`는 무센서를 뜻하지 않는다. Joint encoder, IMU와 state estimator를 사용하되 camera·LiDAR terrain map을 쓰지 않는다는 의미. History도 아직 밟지 않은 terrain을 미리 보지 못하고, 이미 일어난 slip·collision·tracking mismatch에서 hidden condition을 추론한다.

Training에는 natural terrain을 그대로 넣지 않았다. Rigid hills, steps, stairs와 friction variation에서 response strategy를 학습했다. 결과 역시 TCN 하나로 설명할 수 없으며 privileged learning, DAgger, curriculum, PMTG, actuator model과 randomization이 함께 사용됐다.
...
두 세대 robot은 kinematics, inertia와 actuator가 다르다. 논문은 같은 generation 안에서는 모든 환경에 같은 controller를 사용하고 환경별 tuning을 하지 않았다고 설명한다.

이 말은 ANYmal-B용 weight 하나가 ANYmal-C에도 그대로 사용됐다는 뜻은 아니다. 각 robot generation에 맞는 model과 actuator network를 사용하는 pipeline으로 이해하는 것이 안전하다.
...
History $H_t$가 필요한 이유는 hidden condition이 시간에 걸친 response로 나타나기 때문이다.

### **2.2 같은 현재 자세라도 과거가 다르면 상황이 다르다**

현재 joint angle과 body orientation이 같아도 다음 두 경우는 다르다.
...
단일 frame만 보면 구분이 어려울 수 있지만 history에는 다음 신호가 남는다.
...
즉 proprioception은 robot state만 재는 센서가 아니다.

> Action을 보냈을 때 몸이 어떻게 반응했는지를 통해 environment를 간접적으로 측정하는 interaction sensor이기도 하다.

### **2.3 그렇다고 cliff를 볼 수 있는 것은 아니다**

Proprioceptive inference는 contact가 일어난 뒤 강하다. 아직 밟지 않은 절벽, 넓은 gap과 위험 obstacle을 멀리서 미리 알 수는 없다.

그래서 논문이 해결하는 것은 **reactive blind locomotion**이지, global terrain mapping이나 foothold planning이 아니다.
...
그림을 data flow에 따라 나누면 두 training stage와 runtime control이 보인다.

### **Training Stage 1: Teacher**

Teacher는 deployable state $o_t$와 privileged terrain/contact state $x_t$를 받는다. Terrain encoder가 $\bar l_t$를 만들고, teacher MLP가 action $\bar a_t$를 출력하며 TRPO로 업데이트된다.

### **Training Stage 2: Student**

Student는 current deployable state $o_t$와 history $H_t$만 받는다. TCN이 inferred latent $l_t$를 만들고, teacher에서 복사한 policy head가 action $a_t$를 출력한다. Supervision은 action imitation $a_t\approx\bar a_t$와 latent imitation $l_t\approx\bar l_t$를 함께 사용한다.
...
Terrain parameter particle마다 procedural rollout을 수행해 traversability를 추정한다. Medium-difficulty particle에 높은 weight를 주고, 이를 resample·perturb해 다음 학습 난이도를 만든다.

이 네 component 중 하나만으로 결과가 나온 것이 아니다. 원문 ablation은 memory, privileged learning과 adaptive curriculum 각각이 필요했음을 보여준다.

---

## **4. Command는 목표 속도가 아니라 목표 방향이다**

앞선 command-conditioned locomotion은 보통 $(v_x,v_y,\omega_z)$의 목표값을 추종했다.

이 논문은 rough terrain에서 가능한 speed가 지형에 따라 달라진다는 점을 반영해 **방향만 명령**한다.
...
Horizontal direction은 다음 unit vector이다.
...
Policy는 가능한 한 command direction으로 진행하되, 실제 speed는 terrain difficulty와 dynamics에 맞춰 정한다.

이 선택은 중요한 claim boundary를 만든다.

> 이 controller는 arbitrary target speed를 정밀 추종하는 policy가 아니라, rough terrain에서 주어진 방향으로 가능한 속도로 진행하는 policy다.

Stop command가 0.5초 동안 유지되면 base gait frequency를 0으로 만들어 FTG를 멈춘다. 방향 command가 들어오거나 disturbance로 base speed가 0.3 m/s를 넘으면 gait frequency를 다시 1.25 Hz로 둔다.

작은 state machine 하나가 남아 있다는 점도 중요하다. 논문은 모든 discrete logic을 완전히 없앤 controller는 아니다.
...
Foot trajectory generator $F(\phi_i)$는 기본 vertical stepping motion을 만든다.

Policy는 각 foot의 3D residual $\Delta r_{f_i,T}$도 출력한다.
...
Policy가 12개 joint target을 완전히 자유롭게 만들 수도 있다. 이 논문은 기본 trot prior를 넣어 search space를 줄였다.
...
이 구조는 sample efficiency와 transfer 안정성을 높이는 대신 gait diversity를 제한할 수 있다. 실제로 논문의 controller는 trot만 보였다.

### **5.3 Foot target은 horizontal frame에서 정의한다**

각 leg의 target은 hip 아래에 붙은 horizontal frame $H_i$에서 정의된다. 이 frame의 z-axis는 gravity 방향과 정렬되고, yaw는 robot base와 같다.

Base가 순간적으로 roll/pitch되어도 foot target frame이 같이 기울지 않으므로 초기 random action과 rough-terrain disturbance에서 target이 덜 불안정해진다.

Foot target은 analytic IK로 joint position target으로 바뀌고, learned actuator model이 포함된 simulation의 joint PD controller가 이를 추종한다.
...
Teacher와 student의 policy head가 받는 current observation에는 다음이 들어간다.
...
Deploy 시 base velocity와 orientation은 IMU와 leg kinematics를 사용하는 state estimator에서 얻는다.

### **6.2 Privileged state $x_t$**

Simulation-only privileged information에는 다음이 들어간다.
...
Height sample은 각 foot 중심에서 반경 10 cm 원 위에 배치된다.

Teacher의 terrain encoder는 $x_t$를 latent $\bar l_t$로 압축한다.
...
그 latent와 deployable observation을 policy head에 넣는다.
...
Command와 robot state는 privileged encoder 입력에서 제외된다. 따라서 $\bar l_t$가 terrain/contact condition에 집중하도록 설계했다.

### **6.3 Teacher reward**

Teacher는 TRPO로 학습된다. Reward는 다음 weighted sum.
...
Linear reward는 projected speed $v_{pr}$가 0.6 m/s에 도달할 때까지 증가하고 그 이상에서는 saturation된다.
...
이는 “무조건 더 빨리”가 아니라 일정 수준까지 command direction 진행을 장려한다.

### **6.4 Teacher를 직접 real robot에 쓰지 못하는 이유**

Teacher가 요구하는 terrain normal, height scan, friction과 exact contact force는 real deployment에서 안정적으로 얻기 어렵다.
...
Student history sample $h_t\in\mathbb{R}^{60}$에는 command, gravity vector, base twist, joint state, leg phase와 frequency가 들어간다.

Actuator용 짧은 joint history와 previous foot target은 current observation $o_t$에는 있지만 long TCN history $h_t$에서는 제외된다.

History는 20 ms 간격으로 쌓인다.
...
TCN encoder는 세 개의 dilated causal convolution과 stride convolution을 번갈아 사용한다.

Causal이라는 것은 future sample을 보지 않는다는 뜻.
...
Dilation을 사용하면 깊이를 과도하게 늘리지 않고 긴 temporal receptive field를 확보할 수 있다.
...
Action만 맞추면 training data에서 같은 output을 내는 shortcut이 가능할 수 있다. Latent supervision은 student encoder가 teacher의 internal condition representation을 닮도록 추가 제약을 준다.

Supplementary ablation에서 latent loss를 뺀 TCN-100은 slope와 disturbance에서는 비슷했지만 step success가 낮았다.

### **7.4 DAgger로 covariate shift를 줄인다**

Ordinary behavior cloning은 teacher trajectory만 학습한다. Student가 작은 오차로 다른 state에 들어가면 학습 data가 없어 error가 누적될 수 있다.

이 논문은 DAgger-style dataset aggregation을 사용한다.

1. Student policy로 rollout한다.
2. Student가 실제 방문한 state를 수집한다.
3. 같은 state에서 teacher action과 latent를 query한다.
4. Labelled sample을 supervised dataset에 추가한다.
5. 확장된 dataset으로 student를 업데이트한다.

즉 training distribution을 teacher가 잘 가는 state가 아니라 **현재 student가 실제로 가는 state** 쪽으로 맞춘다.

### **7.5 Teacher policy head를 복사한다**

Teacher에서 terrain latent 이후의 MLP는 student로 복사된다. Student의 핵심 학습 대상은 privileged latent를 history에서 만들어내는 TCN encoder.

이 구조는 action semantics를 teacher와 student 사이에 정렬하지만, teacher latent가 반드시 유일하거나 물리적으로 해석 가능한 state라는 뜻은 아니다.
...
Simulation은 deformable mud나 vegetation을 직접 만들지 않는다.
...
Terrain은 매 episode 다른 random seed로 다시 생성된다.
...
Terrain parameter $c_T$의 traversability는 그 terrain에서 success label의 평균.
...
학습에 좋은 terrain은 너무 쉽지도, 너무 어렵지도 않은 구간이다.
...
Terrain type마다 particle 10개를 유지하고, particle당 6 trajectory를 생성한다. 10번 policy iteration마다 traversability를 모아 weight를 갱신한다.
...
High-desirability particle을 resample하고 parameter grid의 인접 값으로 random walk한다. Replay memory는 이전 terrain을 일부 다시 뽑아 catastrophic forgetting을 줄인다.
...
이 curriculum은 “난이도를 일정 비율로 선형 증가”시키는 방법과 다르다. 현재 policy 성능에 따라 parameter distribution이 움직인다.
...
5편과 동일하게 joint-level SEA dynamics는 learned actuator network로 모델링한다.
...
ANYmal-B와 C의 actuator가 다르므로 robot별 actuator model이 필요하다.

### **9.2 Friction, disturbance와 observation noise**

Training에서는 foot-ground friction을 randomize하고, external disturbance와 observation noise를 넣는다. Teacher는 random physical quantity 일부를 privileged input으로 받는다.

따라서 natural-terrain generalization을 TCN 하나의 효과로만 돌리면 안 된다.
...
전체 조합이 transfer stack이다.

### **9.3 Runtime**

Student neural-network controller는 onboard Intel i7-5600U CPU에서 TensorFlow C++ API로 400 Hz 실행된다.

TCN history는 20 ms 간격으로 저장되므로 50 Hz temporal sample grid를 사용하지만, controller inference와 low-level tracking loop는 더 높은 rate로 실행될 수 있다.

Policy output은 joint torque가 아니라 foot residual과 frequency. Analytic IK와 joint PD가 실제 joint target을 계산·추종한다.
...
Training terrain에는 deformability, flowing water와 vegetation이 없었다. 따라서 이 결과는 test environment가 training simulator parameter range 안에 정확히 들어 있었다는 의미의 일반적인 domain randomization 결과와는 다르다.
...
논문은 기존 ANYmal model-based controller와 speed 및 mechanical cost of transport, COT를 비교했다.
...
Mechanical COT가 낮을수록 unit weight와 unit speed당 positive mechanical work가 적다.

단, baseline 숫자는 성공적으로 이동한 구간만 측정했다. Vegetation에서는 catastrophic failure 때문에 비교값을 만들지 못했고, baseline이 실패하면 사람이 더 안정적인 pose로 reset했다.

따라서 표만 보면 controller 차이를 오히려 과소평가할 수 있다. 반대로, 실험 횟수와 terrain condition이 대규모 표준 benchmark처럼 통제된 것도 아니므로 universal superiority로 확대하면 안 된다.

### **10.2 DARPA Subterranean Challenge**

이 controller는 CERBERUS team의 DARPA Subterranean Challenge Urban Circuit에서 이전 model-based controller를 대체했다.
...
Laboratory clip이 아니라 mission-duration deployment에서 failure 0회를 기록한 것이 이 논문의 중요한 강점.

다만 robot 전체 system failure나 navigation performance까지 0이었다는 뜻은 아니다. 논문 표현은 **locomotion controller의 zero failure rate**이다.
...
Natural-environment video만으로는 어떤 mechanism이 중요한지 분리하기 어렵다. 그래서 논문은 step, payload와 slippery surface를 통제해 실험했다.

### **11.1 Emergent foot-trapping response**

16.8 cm step에 swing foot가 부딪혔을 때 controller는 다음 swing에서 foot를 더 높이 들었다.
...
Hind-leg clearance도 증가했다. Front leg가 step을 만났다는 과거 정보가 뒤이어 오는 hind-leg trajectory에 영향을 준 셈이다.

Mid-shin collision에도 대응했다. 즉 explicit foot-contact event 하나에 묶인 scripted reflex는 아니었다.

논문은 이를 foot-trapping reflex라 부르지만, reward에 “발이 걸리면 이렇게 들어라”라는 규칙을 직접 넣지는 않았다. Procedural steps와 history-based control에서 emergent하게 나온 response.

### **11.2 10 kg unseen payload**

ANYmal-B에 10 kg payload를 부착했다. 이는 total robot weight의 22.7%였고 training에서 simulate하지 않은 mismatch였다.
...
이 결과에는 terrain response와 model mismatch robustness가 함께 들어 있다.

### **11.3 Wet whiteboard slip**

Moistened whiteboard로 foot slippage를 만들었다.

- Baseline은 balance를 잃고 aggressive leg swing 후 fall
- Proposed controller는 commanded direction으로 계속 locomotion

Policy는 explicit friction estimator를 control loop에 사용하지 않는다. 이후 diagnostic decoder로 TCN representation을 분석했을 때 first slip 이후 estimated friction이 낮아지는 pattern을 복원할 수 있었다.
...
Diagnostic test는 slope, step과 external force 세 가지. Step·slope 조건마다 100 trial을 수행했고 friction은 $U(0.4,1.0)$에서 샘플링했다.

### **12.1 Memory length**

Slope에서는 history length 차이가 크지 않았다. 지속적인 slope는 current pose와 gravity vector만으로도 어느 정도 대응할 수 있기 때문이다.

Step과 disturbance에서는 차이가 컸다.
...
External force는 5초 동안 적용했다.

이 결과는 “memory가 항상 모든 terrain에서 중요하다”보다 다음처럼 읽는 것이 정확하다.
...
즉 student architecture가 약해서가 아니라 sparse·difficult rough-terrain RL problem을 deployable observation만으로 처음부터 푸는 것이 어려웠다.

Privileged teacher는 더 informative한 state에서 먼저 locomotion을 학습하고, student가 그 solution을 따라가게 해 optimization problem을 나눴다.
...
Uniform sampling은 현재 policy가 거의 통과할 수 없는 terrain을 자주 뽑아 episode가 빨리 끝났다. Adaptive curriculum은 성공 가능한 frontier에서 더 긴 trajectory와 유용한 learning signal을 만들었다.

### **12.4 TCN vs GRU와 latent-loss ablation**

Supplementary experiment에서는 architecture와 latent loss의 영향을 따로 비교했다.
...
GRU도 유효했으므로 TCN만 가능한 문제는 아니다. 다만 이 setup에서는 긴 TCN이 step handling과 training efficiency에 더 유리했다.
...
저자들은 trained TCN의 intermediate representation을 고정하고, 별도의 decoder를 나중에 학습했다.

Frozen representation에서 decoder가 복원한 대상은 아래와 같다.
...
Regression은 mean과 uncertainty를 출력하며 Gaussian negative log-likelihood를 사용했다.
...
Decoder는 policy training이나 real control에 들어가지 않은 **representation probing 도구**.
...
_A는 16.8 cm step response, B는 latent에서 decode한 terrain/contact, C-D는 foot target height에 대한 input-history saliency다. FT는 최초 foot collision 시점. 출처: [Lee et al., Figure 6](https://arxiv.org/pdf/2010.11251)._
...
Saliency map에서는 first collision 시점의 left-front joint position, velocity와 position-error channel이 이후 foot-clearance output에 계속 영향을 주었다.

이것은 TCN이 과거 collision event를 활용한다는 강한 정황이다.

그러나 saliency와 decoder reconstruction은 causal proof가 아니다.

- Decoder가 representation에 있는 상관관계를 읽었을 수 있다.
- Latent의 특정 component가 policy action에 반드시 필요한지는 별도 intervention이 필요하다.
- Real terrain에서 decoded geometry가 정확하다고 직접 검증한 것은 아니다.
...
Student가 2초 history를 사용하는 이유와 이 recovery timing이 연결된다.
...
Teacher의 주요 hyperparameter는 아래와 같다.
...
Student는 Adam으로 4,000 iteration 학습되고, TCN batch size는 20,000.

이 숫자는 당시 custom RaiSim pipeline의 결과. 다른 simulator, GPU와 vectorized environment 수에서 wall-clock을 직접 비교하면 안 된다.
...
Policy는 natural deployment terrain에서 reward를 받아 fine-tuning하지 않았고, mud·snow·vegetation별 parameter tuning도 하지 않았다.

따라서 environment-level zero-shot transfer라는 표현은 적절하다.

### **15.2 무엇이 완전한 zero-data는 아닌가**

- Robot의 CAD/rigid-body model이 필요하다.
- Robot별 SEA actuator network를 real data로 학습한다.
- State estimator와 joint PD controller가 필요하다.
- Reward, PMTG와 terrain generator를 사람이 설계한다.

즉 real-world task data를 사용하지 않았다는 것과 real robot data·engineering이 전혀 없다는 것은 다르다.

### **15.3 왜 simple simulation이 natural terrain으로 확장됐는가**

논문 결과만으로 mechanism을 하나로 확정할 수는 없다. 다만 ablation과 probing을 함께 보면 아래 연결이 가장 자연스럽다.
...
Natural terrain의 픽셀이나 geometry가 training과 같아서가 아니라, **controller가 처리해야 하는 proprioceptive event class가 겹쳤기 때문**이라고 보는 편이 타당하다.
...
1. Long proprioceptive history는 step과 disturbance 대응을 개선했다.
2. Deployable observation만으로 direct RL한 baseline은 rough-terrain locomotion을 학습하지 못했고, privileged training은 성공했다.
3. Adaptive terrain curriculum은 uniform sampling보다 test success, reward와 episode length를 개선했다.
4. Controller는 simulation에 없던 mud, snow, rubble, vegetation과 water에서 동작했다.
5. DARPA field deployment에서 4회의 60분 mission 동안 locomotion failure 0회를 기록했다.
6. 10 kg unseen payload, wet board와 16.8 cm foot trapping에 controlled robustness를 보였다.

### **16.2 조심해서 말해야 하는 것**

1. **Terrain을 정확히 reconstruct했다:** Decoder는 probe이며 deploy policy가 explicit map을 쓰는 것이 아니다.
2. **Vision/LiDAR가 필요 없다:** Blind locomotion은 unseen cliff나 gap을 피할 수 없다.
3. **모든 자연환경에 generalize한다:** 보고된 deployment set에서의 evidence.
4. **Teacher 없이 불가능하다:** 이 architecture와 training budget의 direct-RL baseline이 실패한 결과.
5. **TCN이 GRU보다 항상 낫다:** 이 실험에서는 TCN-100이 step과 update time에서 유리했다.
6. **RL controller가 logic을 전혀 쓰지 않는다:** Stand/locomotion 전환용 작은 state machine과 fixed FTG가 있다.
...
논문 표현대로 cliff 방향으로 command하면 걸어갈 수 있다.

Contact 후 반응하는 능력은 뛰어나지만, 치명적 terrain을 contact 전에 피하려면 exteroception이 필요하다.

### **17.2 Gait diversity가 제한된다**

Controller는 trot만 보였다. PMTG의 fixed phase structure와 reward가 다른 gait 발견을 제한했을 수 있다.

더 빠른 terrain에서는 gallop, 느린 terrain에서는 crawl처럼 gait를 바꾸는 결과는 보여주지 않는다.

### **17.3 Conservative locomotion**

Terrain을 몸으로 느껴야 하므로 gait가 조심스럽고 speed가 낮다. Safe terrain에서 exteroception을 사용하면 더 빠르고 효율적인 locomotion이 가능할 수 있다.
...
단일 end-to-end policy보다 component가 많고 debugging point도 많다.

### **17.5 Privileged state 설계에 expert knowledge가 들어간다**

Teacher가 local height, normal, contact, friction과 disturbance를 보도록 사람이 정했다. 어떤 privileged variable을 넣느냐가 teacher behavior와 student latent를 바꾼다.
...
Actuator model, estimator, PMTG, IK와 400 Hz control loop가 flat ground에서 안정적인지 먼저 확인한다.

### **Step 2. Teacher privileged state 검증**

Terrain height와 normal의 frame, contact state, force와 friction unit을 하나씩 확인한다.
...
Frame mismatch는 teacher가 simulation exploit을 배우게 만들 수 있다.

### **Step 3. Curriculum 없이 작은 terrain**

거의 flat hills와 낮은 step에서 teacher reward와 termination을 검증한다. Particle filter를 붙이기 전에 single-terrain overfit test를 통과해야 한다.

### **Step 4. Adaptive curriculum**

Particle별 traversability, weight, resampling과 replay ratio를 log한다.

Pass criterion은 terrain parameter가 무작정 max difficulty로 몰리는 것이 아니라 policy 성능과 함께 frontier가 이동하는 셈이다.
...
Offline validation만으로 끝내지 말고 student rollout에서 DAgger query가 실제로 늘어나는지 확인한다.

### **Step 6. Diagnostic simulation ablation**

TCN-1, TCN-20, TCN-100을 slope, step, 50 N disturbance에 비교한다. Natural terrain video보다 mechanism debugging에 유용하다.
...
Torque·joint-speed limit, emergency stop, fall harness와 operator clearance를 먼저 둔다.

### **Step 8. Exteroception과 결합**

Blind policy를 fallback base로 유지하고, valid terrain estimate가 있을 때만 exteroceptive residual을 추가하는 구조가 현실적.
...
Hwangbo et al.은 짧은 joint history로 actuator의 delay와 hidden response를 model 안에 넣었다.

### **6편: terrain/contact hidden state**

Lee et al.은 2초 proprioceptive history로 terrain, contact와 disturbance의 흔적을 policy 안에 넣었다.

### **7편: explicit adaptation vector**

다음 RMA는 environment dynamics를 privileged encoder가 explicit latent $z_t$로 만들고, adaptation module이 recent history에서 그 latent를 빠르게 예측하도록 구조화한다.
...
6편은 **history가 online adaptation mechanism이 될 수 있다**는 흐름을 real natural terrain에서 강하게 보여준 연결점.

---

## **20. Terrain Map보다 먼저 Body Response를 읽는다**

Privileged teacher는 terrain profile, contact, friction과 disturbance를 보며 rough-terrain response를 배웠다. Student는 deployable observation과 2초 proprioceptive history만으로 teacher action과 latent를 함께 모방했고, DAgger로 자신이 실제 방문하는 state까지 training data에 포함했다. PMTG의 leg frequency와 foot residual을 policy가 조절하고, IK와 joint PD가 이를 actuator command로 바꿨다.

Particle-filter curriculum은 traversability 0.5-0.9의 learning frontier를 따라갔다. Long memory, privileged training과 adaptive curriculum은 각각 ablation에서 이득을 보였다. 실제 ANYmal은 mud, snow, vegetation, rubble와 running water에서 동작했고, controlled test에서는 16.8 cm foot trapping, 10 kg payload와 wet surface를 따로 평가했다.

Decoder와 saliency는 latent에 hidden-condition information이 있다는 근거이지 explicit terrain reconstruction의 증명은 아니다. 또한 blind control은 contact 전에 cliff나 gap을 피하는 문제를 해결하지 못한다.
...
다음 편의 **Rapid Motor Adaptation, RMA**는 이 hidden-condition inference를 explicit environment latent와 adaptation module로 분리한다.
