...
date: 2026-03-04 21:03:00 +0900
last_modified_at: 2026-03-04 21:03:00 +0900
...
발을 들도록 유도하는 reward를 여러 방식으로 추가한 결과, 이제는 MuJoCo에서는 Go2가 걸을 수 있게 됐다.
...
이번에 사용한 reward는 크게 세 가지였다.

1. `feet_air_time`: 발을 threshold보다 오래 들었을 때 보상을 주는 항.
   1. weight를 `0.01`에서 `5.0`으로 변경했다.
1. `feet_clearance`: height scanner를 기준으로 발을 target height만큼 들어 올렸을 때 보상을 주는 항.
   1. 움직이는 발의 x, y 좌표에서 가장 가까운 height scanner 값을 기준으로 비교했고, target height는 `0.08 m`로 설정했다.
   <details markdown="1">
...
1. `feet_body_height`: base frame을 기준으로 발이 특정 높이까지 올라오도록 유도하는 항이다.
   1. Go2의 default pose에서 body height가 약 35 cm이므로, 지면으로부터 약 8 cm를 목표로 잡아 target height를 `-0.27`로 설정했다.
   <details markdown="1">
...
1. Go2는 `feet_slide` weight에 상당히 민감하게 반응했다. penalty를 과하게 키우면 오히려 발을 떼지 않는 문제가 생겼다.
1. 이를 보완하기 위해 `feet_body_height` reward를 설계했다.
   1. 하지만 target height를 높게 설정하는 순간, robot이 발을 드는 대신 base height를 낮추는 문제가 발생했다.
      ![](/assets/img/posts/unitree/sim2real/unitree-go2-part-3-sim2real-is-hard/32acbb7d-7937-80eb-a498-f16ed8ec48fd.gif)
1. `feet_clearance` reward에서 target height를 `0.08 m`로 두고 학습하자, 발을 옆으로 들어 `air_time`과 `clearance` reward를 동시에 키우려는 local minimum이 나타났다.
   ![](/assets/img/posts/unitree/sim2real/unitree-go2-part-3-sim2real-is-hard/32acbb7d-7937-8095-8563-fd5600868e80.gif)

이 단계에서는 `feet_air_time`을 빼고 `feet_clearance` 중심으로 학습하는 방향을 선택했다.
...
또한 로봇이 stand 상태에서 다리를 많이 접는 현상이 있었다. 이 문제는 `last_action` observation 구성 버그와 관련이 있었다.

`last_action`에는 ONNX 모델의 raw action이 들어가야 했지만, deploy 코드에서는 scale과 offset이 적용된 target position에 가까운 값이 들어가고 있었다. 이 때문에 policy가 training 때와 다른 형태의 action history를 보고 있었고, 이전 action이 지나치게 보수적으로 들어가는 문제가 생겼다. 이 부분을 수정하자 로봇 다리가 펴지는 것을 확인할 수 있었다.
...
Go2에 deploy된 모델의 torque 값을 확인해보니, 뒷다리 torque가 앞다리에 비해 비정상적으로 낮게 나오는 것을 확인했다.

![](/assets/img/posts/unitree/sim2real/unitree-go2-part-3-sim2real-is-hard/32acbb7d-7937-80d3-8e0f-cac3ab04cc22.webp)

가능한 원인은 두 가지로 봤다.

1. terrain height를 너무 높게 잡아, 앞발이 Go2 전체를 끌어가는 형태로 학습되었을 수 있다.
1. position error 기반의 PD target만으로는 Go2가 실제로 필요한 torque를 만들기 어려웠을 수 있다.

deploy 시 뒷다리 stiffness를 `25`에서 `40`으로 변경하자, 로봇이 앞뒤좌우로 조금씩 걷기 시작했다.
...
- 앞뒤로 움직일 때 다리를 너무 높게 드는 문제가 있었다.
  - 뒷다리 stiffness를 임의로 높이면서 torque가 강하게 나왔고, 그 결과 앞다리에 비해 뒷다리가 과하게 올라갔다. 이는 Go2의 balance를 무너뜨려 안정적인 제어를 어렵게 만들었다.


- command를 `0.5 m/s` 이상으로 줘야만 로봇이 움직이기 시작했다.
  - 실제로 움직이기 위해 필요한 torque margin이 높다는 뜻.


## **6. 다음 방향**

이 시점에서 reward 조정만으로는 부족하다고 판단했다. simulation에서는 통과하지만 real에서는 torque가 부족하거나 target tracking이 흔들리는 문제가 남아 있었기 때문.

다음 글에서는 이 문제를 줄이기 위해 feed-forward torque를 추가하는 방향을 실험한다.

1. 기존 구조에서는 로봇이 자기 몸무게를 버티기 위해 policy가 목표 관절 각도를 크게 틀어야 했다. simulation에서는 이런 방식이 어느 정도 통했지만, real robot에서는 actuator가 그 target을 안정적으로 따라가지 못했다.
1. PhysX에서 계산한 gravity compensation torque를 사용해, 현재 자세에서 중력을 버티는 데 필요한 기본 torque를 feed-forward로 먼저 넣어본다.
1. 기대한 효과는 policy가 몸을 억지로 띄우기 위한 과격한 target 대신, 더 부드러운 발 궤적을 출력하도록 만드는 것이다.
