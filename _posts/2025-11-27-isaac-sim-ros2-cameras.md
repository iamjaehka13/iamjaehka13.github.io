...
[이전 TurtleBot ROS2 연결 글](/posts/isaac-sim-turtlebot-ros2/)에서 만든 stage에 카메라 두 대를 추가했다. Camera prim만 배치해서는 ROS2 image가 생기지 않는다. **Camera prim에서 render product를 만들고, ROS2 Camera Helper가 이를 image topic으로 내보내야** RViz2에서 영상을 볼 수 있다.
...
먼저 이전 글에서 import한 TurtleBot의 body 아래에 `camera1`, `camera2` prim을 만든다. 카메라는 반드시 로봇의 움직임을 따라가는 prim 아래에 있어야 한다.

URDF import 후 namespace가 붙은 경우에는 보통 `a__namespace_base_footprint`처럼 이름이 붙은 base prim 아래에 카메라를 추가한다. 이렇게 해야 로봇이 움직일 때 카메라 frame도 같이 따라간다.
...
두 카메라의 transform은 로봇 기준 앞뒤를 바라보도록 잡는다. 한쪽은 전방, 다른 한쪽은 후방을 보게 만들면 ROS2 topic이 정상적으로 분리되는지 확인하기 쉽다.
...
또는 `Tools > Sensors > Camera Inspector`를 열면 scene 안에 있는 카메라들을 한 번에 확인할 수 있다.

## **2. ROS2 Camera OmniGraph 만들기**

이제 시뮬레이션 카메라 이미지를 ROS2 topic으로 내보낸다. Isaac Sim에는 camera용 ROS2 OmniGraph shortcut이 있어서, 처음부터 모든 노드를 수동으로 만들지 않아도 된다.
...
여기서 render product는 Isaac Sim 내부 camera output을 ROS2 helper가 사용할 수 있는 형태로 만들어주는 중간 단계라고 보면 된다. 그래서 카메라 topic이 나오지 않을 때는 camera prim target, render product 연결, helper의 topic 설정을 차례대로 확인하는 것이 좋다.

## **3. RViz2에서 카메라 Topic 확인**

Isaac Sim stage를 play한 뒤 host 터미널에서 ROS2 환경을 source하고 topic 목록을 확인한다.
...
이 방식은 이후 depth image나 camera info를 추가할 때도 그대로 확장된다. sensor prim을 만들고, render product를 만들고, ROS2 helper가 원하는 message type으로 publish하는 패턴.
...
이 예제는 실제 카메라 연결 과정과는 조금 별개지만, Isaac Sim에서 생성한 image stream을 ROS2로 publish하고 RViz2에서 확인하는 구조를 다시 한 번 확인하기 좋다.
...
같은 graph에 depth image와 camera info publisher를 추가할 수 있고, lidar도 helper 종류만 달라질 뿐 연결 방식은 비슷하다. 센서가 늘어나면 prim target과 topic name을 먼저 표로 정리해두는 편이 연결 실수를 줄여준다.
