...
벨만 방정식이 관계를 정의해도 최적 정책이 저절로 계산되는 것은 아닙니다. 장기 반환값을 최대로 만드는 **최적 정책($\pi^{\ast}$)**을 실제로 구하려면 반복 계산이 필요합니다. 사용할 수 있는 계산 방식은 환경 모델을 알고 있는지에 따라 달라집니다.
...
이 예시에서는 정책 반복의 두 연산이 번갈아 나타납니다.
...
그림을 본문의 계산 과정과 연결하면 다음 세 항목으로 읽을 수 있습니다.
...
정책 반복은 평가와 개선을 분리하고, 가치 반복은 벨만 최적 백업 안에서 두 과정을 더 촘촘하게 섞습니다. 어느 쪽이든 상태 전이 확률 $P$와 보상 함수 $R$을 알고 있다는 전제는 같습니다. 실제 문제에서는 이 모델을 정확히 적기 어려운 경우가 더 많으므로, 전체 다음 상태를 합산하는 대신 관측한 transition으로 값을 추정해야 합니다. 이 지점부터 동적 계획법이 강화학습으로 이어집니다.
