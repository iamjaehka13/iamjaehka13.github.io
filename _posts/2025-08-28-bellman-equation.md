...
모든 미래 궤적을 직접 펼쳐서 비교할 수는 없습니다. **벨만 방정식(Bellman Equation)**은 이 긴 시간축을 한 단계의 재귀 관계로 바꿉니다.

> 미래 전체를 한 번에 계산하지 말고, **현재 보상 + 다음 상태의 가치**로 현재 상태의 가치를 재귀적으로 표현하자.

에이전트는 다음 상태 $s^{\prime}$의 가치 $v(s^{\prime})$를 이용해 현재 상태를 평가할 수 있습니다. 다음 상태의 가치를 현재 추정치로 가져오는 연산을 백업(Backup)이라고 부릅니다. 이 재귀 구조 덕분에 먼 미래의 보상도 한 step씩 현재 선택에 반영됩니다.
...
현재 상태의 가치는 다음 상태 가치들의 기댓값으로 표현할 수 있습니다. 이 재귀적 관계는 두 가지 개념과 연결됩니다.
...
현재 정책에 따라 행동했을 때 얻는 보상과 다음 상태 가치의 기댓값을 더하므로, 벨만 기대 방정식은 정책 평가(Policy Evaluation)의 기본 도구가 됩니다.
...
벨만 기대 방정식은 현재 정책을 평가하고, 벨만 최적 방정식은 더 좋은 행동을 선택하는 기준을 제공합니다. 두 식의 차이보다 더 실용적인 분기점은 환경 모델의 유무입니다. 전이 확률과 보상 함수를 알고 있다면 값을 직접 계산할 수 있고, 모른다면 실제 transition을 모아 같은 관계를 추정해야 합니다. 다음 글에서는 전자인 **Known MDP**에서 동적 계획법(Dynamic Programming)이 어떻게 반복 계산을 수행하는지 이어서 보겠습니다.
