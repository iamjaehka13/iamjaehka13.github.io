...
1주차에는 frame과 SE(3)를 공부했다. 핵심은 LiDAR pose를 다음처럼 계산하는 것이었다.
...
LiDAR SLAM에서 point cloud 하나를 한 순간의 사진처럼 보면 안 된다. LiDAR scan 하나는 일정 시간 동안 순차적으로 측정된 point들의 묶음.

예를 들어 10 Hz LiDAR라면 scan 하나는 약 0.1초 동안 만들어진다.
...
이 0.1초 동안 로봇이 움직이면 scan 초반 point와 scan 후반 point는 서로 다른 LiDAR pose에서 측정된다. 그래서 point별 timestamp를 모르면 deskew도 정확히 할 수 없다.

이번 주 목표는 SLAM 알고리즘을 돌리는 것이 아니라, LiDAR measurement time을 감사하는 도구를 만드는 것이다.
...
LiDAR scan은 보통 한 주기 동안 모은 point들의 묶음.
...
예를 들어 같은 cloud 안에서도 point별 측정 시각은 다를 수 있다.
...
로봇이 정지해 있으면 이 차이가 크게 문제 되지 않을 수 있다. LiDAR pose가 scan 동안 거의 변하지 않기 때문.

하지만 Go2 같은 legged robot은 걷는 동안 base가 계속 흔들린다. LiDAR가 base 위에 올라가 있으므로 scan 동안 LiDAR pose도 변한다.
...
이걸 모두 같은 pose에서 찍힌 것처럼 처리하면 cloud가 휘어진다. 이 motion distortion을 펴는 과정이 deskew.

## **2. Packet과 PointCloud2**

LiDAR는 point를 한 번에 전부 보내지 않는다. 센서 내부에서 측정한 데이터를 작은 묶음으로 나눠 보낸다. 이 묶음을 packet이라고 보면 된다.
...
즉 packet 여러 개가 모여 PointCloud2 하나가 된다.
...
PointCloud2 header에는 timestamp가 하나만 있다. 하지만 cloud 안의 point들은 서로 다른 시간에 측정됐다.

그래서 2주차에서 가장 먼저 확인해야 하는 것은 다음이다.
...
ROS `sensor_msgs/PointCloud2`에는 보통 다음 header가 있다.
...
`header.stamp`는 cloud의 대표 timestamp. 하지만 이 값이 항상 scan 시작 시각이라는 보장은 없다.
...
예를 들어 scan duration이 0.1초이고 실제 scan 구간이 다음과 같다고 하겠다.
...
`header.stamp`가 scan start라면 point 상대시간을 바로 더할 수 있다.
...
반대로 `header.stamp`가 scan end라면 scan start를 먼저 복원해야 할 수 있다.
...
즉 header 기준이 무엇인지 모르면 point의 절대 측정 시간이 틀어진다.

## **4. Point Time 단위 확인**

point time field가 있어도 단위를 반드시 확인해야 한다.
...
단위를 잘못 해석하면 scan duration이 말도 안 되게 커지거나 작아진다. 그러면 IMU margin, deskew, latency 판단이 모두 깨진다.
...
이런 상태에서는 어떤 SLAM 결과도 믿기 어렵다.

## **5. Ring, Channel, Point Ordering**

multi-channel LiDAR는 여러 개의 laser channel을 가진다.

예를 들어 16채널 LiDAR라면 대략 다음처럼 생각할 수 있다.
...
mechanical spinning LiDAR에서 `ring`은 보통 몇 번째 수직 laser layer인지 나타낸다.

SLAM 시스템 중에는 point cloud를 range image처럼 정리할 때 ring 정보를 쓰는 경우가 있다.
...
그래서 일부 LIO 시스템은 point별 time뿐 아니라 ring 정보도 요구한다.

센서나 driver에 따라 이름은 다를 수 있다.
...
2주차에는 실제 PointCloud2 field list를 확인해야 한다.
...
point ordering은 PointCloud2 안에서 point들이 저장된 배열 순서다.
...
하지만 ring별 정렬이면 전체 배열 기준으로 time이 감소할 수 있다.
...
여기서 index 2에서 3으로 갈 때 time이 줄어든다.

```text
0.01 < 0.20
```

이것은 꼭 센서 오류라는 뜻은 아니다. driver가 ring별로 재정렬했을 수 있다.

그래서 `timestamp_monotonicity` 검사는 이렇게 해석해야 한다.
...
point별 time field가 정확하다면 배열 순서가 시간순이 아니어도 괜찮다. 각 point의 실제 time을 직접 쓰면 된다.

## **7. 왜 Deskew가 필요한가**

deskew는 움직이면서 찍은 LiDAR scan을, 마치 한 순간에 찍은 scan처럼 펴주는 과정.

LiDAR scan 하나는 여러 시간에 걸쳐 만들어진다.
...
로봇이 scan 동안 움직이면 각 point가 측정된 시각의 LiDAR pose가 다르다.
...
deskew는 각 point를 자기 측정 시각의 LiDAR pose에서 기준 시각의 LiDAR pose로 변환한다.
...
deskew가 틀리면 cloud 자체가 휘어진다. 그 결과 scan-to-map matching, camera projection, map sharpness가 모두 영향을 받을 수 있다.
...
deskew에는 point time뿐 아니라 그 시각의 motion이 필요하다. 보통 IMU 또는 odometry를 사용한다.

문제는 LiDAR timestamp와 IMU timestamp가 같은 시간축에 있지 않을 수 있다는 점이다.
...
핵심 질문은 단순히 단위가 초인지가 아니다.

> LiDAR time 10.050초와 IMU time 10.050초가 같은 실제 순간을 의미하는가?

예를 들어 실제로 LiDAR timestamp가 IMU보다 20 ms 늦게 붙는다면, deskew는 항상 20 ms 뒤의 IMU motion을 사용하게 된다.
...
Go2처럼 body pitch/roll/yaw가 흔들리는 플랫폼에서는 수 ms offset도 결과에 영향을 줄 수 있다.

### **Clock Model로 보면 더 명확하다**

LiDAR timestamp와 IMU timestamp가 모두 초 단위 숫자로 보인다고 해서 같은 clock이라는 뜻은 아니다.

센서 time을 ROS time으로 옮기는 과정을 단순화하면 다음처럼 볼 수 있다.
...
2주차에서 가장 먼저 보는 것은 보통 offset $b$.

하지만 긴 bag이나 hardware sync가 약한 시스템에서는 scale $a$도 문제가 될 수 있다.
...
deskew에서는 이 차이를 구분해야 한다.

offset은 한 번 보정하면 줄일 수 있지만, jitter는 scan마다 남고, drift는 시간 구간별로 달라질 수 있다.

### **시간 오차가 공간 오차로 바뀌는 방식**

timestamp error가 왜 위험한지 수식으로 보면 더 직관적이다.

point time이 $\delta t$만큼 틀렸다고 하겠다.

LiDAR가 선속도 $\mathbf{v}$와 각속도 $\boldsymbol{\omega}$로 움직이고 있다면, 1차 근사로 point 위치 오차는 다음처럼 볼 수 있다.
...
여기서 $r$은 LiDAR에서 point까지의 거리.

translation에 의한 오차는 속도와 시간 오차에 비례한다.

rotation에 의한 오차는 여기에 range가 곱해진다.

예를 들어 point가 10 m 앞에 있고, 각속도가 `100 deg/s`라고 하겠다.
...
약 8.7 cm point 위치 오차가 생길 수 있다.

그래서 legged robot에서는 "몇 ms 정도는 괜찮겠지"라고 생각하면 위험하다.

body pitch/roll/yaw가 빠르게 흔들릴수록, 그리고 멀리 있는 구조물을 볼수록 timestamp alignment가 더 중요해진다.

## **9. IMU가 Scan을 덮는다는 뜻**

LiDAR scan 시간 구간이 다음과 같다고 하겠다.
...
deskew를 하려면 IMU data가 이 구간 전체를 덮어야 한다.
...
그래야 scan 내부 모든 point 시각에 대해 motion을 추정할 수 있다.

이를 margin으로 저장할 수 있다.
...
반대로 `imu_end_margin`이 음수이면 scan 끝부분 point를 보정할 IMU가 부족하다.
...
이 경우 scan 끝 20 ms 구간은 IMU coverage가 부족하다.

margin이 음수라고 해서 항상 IMU 센서가 나쁜 것은 아니다. 가능한 원인은 여러 가진다.
...
따라서 margin은 원인 확정이 아니라 timestamp/coverage 문제를 드러내는 진단 지표로 봐야 한다.

### **Interpolation에 필요한 coverage**

IMU가 scan 구간 안에 sample을 가지고 있다는 것만으로 충분하지 않을 때가 많다.

point time $t_i$에서 angular velocity나 orientation을 interpolate하려면 보통 $t_i$의 앞뒤 sample이 필요하다.

```text
imu[k].time <= t_i <= imu[k + 1].time
```

따라서 scan 시작과 끝에서 약간의 여유가 필요하다.
...
이 여유가 없으면 extrapolation을 해야 한다.

extrapolation은 interpolation보다 위험하다.
...
짧은 extrapolation은 큰 문제가 아닐 수 있지만, foot impact나 빠른 body rotation이 있는 scan에서는 scan 끝 몇 ms도 무시하기 어렵다.

그래서 auditor에서 `imu_start_margin`, `imu_end_margin`을 따로 저장하는 것이 중요하다.

## **10. Jitter, Packet Loss, Buffering Latency**

2주차에는 timestamp 품질도 봐야 한다.

### **10.1 Timestamp Jitter**

10 Hz LiDAR라면 header stamp 간격은 대략 0.1초여야 한다.
...
그래서 scan 간격의 평균, 표준편차, min/max를 봐야 한다.

### **10.2 Point Time Jitter**

scan 내부 point time도 확인해야 한다.
...
단, point ordering이 시간순이 아닐 수 있으므로 전체 배열 기준 diff만 보고 오류라고 단정하면 안 된다. ring별 또는 packet별 ordering일 수 있다.

### **10.3 Packet Loss**

packet loss는 LiDAR packet 일부가 누락되는 것.
...
처음 구현에서는 단순한 `dropped_packet_flag`부터 만들어도 된다.
...
buffering latency는 실제 측정 시간과 ROS에서 message를 받은 시간 사이의 지연이다.
...
이 경우 약 25 ms 늦게 message가 도착한 것.

이 자체가 항상 문제는 아니다. 하지만 `header.stamp`가 실제 측정 시각이 아니라 ROS receive time이라면 deskew에는 큰 문제가 된다.
...
이번 주 구현 과제는 `lidar_time_auditor`이다.

목표는 각 scan마다 timestamp 관련 정보를 CSV 또는 JSONL로 저장하는 것.
...
실제 구현에서는 `extract_point_times()`가 가장 중요하다. 센서마다 field 이름과 단위가 다르기 때문이다.
...
이 이름들을 실제 PointCloud2 field list에서 확인해야 한다.

## **12. 실험 프로토콜**

2주차 마지막에는 실제 데이터에서 point time과 IMU time이 정상인지 봐야 한다.

실험은 복잡하게 시작하지 말고 네 가지면 충분하다.
...
위 프로토콜을 실제 Unitree LiDAR rosbag에 적용해 봤다.
...
즉 이 데이터의 `PointCloud2`에는 point별 `time` field가 존재했다. 따라서 scan 내부의 최소/최대 point time으로 scan 측정 구간을 직접 계산할 수 있다.
...
여기서 먼저 볼 점은 scan duration이다. 이 rosbag에서는 한 scan이 약 `0.064 s` 동안 측정됐고, cloud header 간격은 약 `0.067 s`이었다.
...
따라서 이 데이터는 10 Hz의 100 ms scan이라고 가정하면 안 된다. 실제 point time field에서 scan 길이를 직접 읽어야 한다.

IMU coverage와 timestamp 품질은 다음처럼 나왔다.
...
첫째, point time은 거의 monotonic하지만 완전히 monotonic하지는 않았다.

```text
monotonic ratio ~= 0.997 ~ 0.998
```

그래서 point array index를 그대로 시간순이라고 믿으면 안 된다. deskew 구현에서는 point별 `time` 값을 기준으로 처리하거나, 최소한 ring/channel별 ordering을 따로 확인해야 한다.

둘째, IMU는 scan 구간을 대체로 잘 덮고 있었다. scan마다 scan 내부 IMU sample median은 16개였고, scan 시작/끝에서 가장 가까운 IMU까지의 최대 margin도 약 5-8 ms 수준이었다.
...
이 정도면 deskew 단계에서 IMU interpolation을 시도할 수 있는 데이터 조건은 갖춰져 있다고 볼 수 있다. 다만 실제 deskew에서는 scan start보다 앞의 IMU와 scan end보다 뒤의 IMU가 모두 필요하므로, bag 시작과 끝부분은 별도로 잘라내는 편이 안전하다.

셋째, walking 구간에서는 stand-like 구간보다 `max_point_time_gap`이 더 크게 관찰됐다.
...
이 값은 point stream 내부에 시간 간격이 크게 벌어진 scan이 있다는 뜻. 이것이 실제 packet loss인지, driver buffering인지, ring/channel ordering의 영향인지는 이 결과만으로 단정하면 안 된다.

넷째, `dropped_packet_flag`는 보수적인 study flag이다.
...
라고 해석하면 안 된다. 여기서는 point 수, scan duration, point time gap이 평소보다 이상한 scan을 표시한 것. 특히 두 번째 walk-like bag에서는 flag ratio가 약 `0.62%`로 더 높았으므로, 해당 scan들을 따로 열어서 point count, ring 분포, time gap 위치를 확인해야 한다.

이번 audit 결과를 2주차 관점에서 정리하면 다음.
...
따라서 다음 단계는 단순히 deskew를 적용하는 것이 아니라, flagged scan을 먼저 시각화하고 ring별 point time 분포를 확인하는 것이다. 그래야 deskew가 실제 motion distortion을 줄이는지, 아니면 timestamp/order 문제를 더 크게 만드는지 분리해서 볼 수 있다.

## **14. Timestamp audit에서 deskew로**

> LiDAR point cloud는 한 순간의 사진이 아니라 시간 구간이고, deskew를 하려면 각 point의 실제 측정 시간을 알아야 한다.

이번 audit에서는 `/utlidar/cloud`의 point별 `time`, 약 64 ms의 실제 scan duration, 약 67 ms의 cloud interval, scan당 IMU coverage를 확인했다. 동시에 point ordering이 완전히 monotonic하지 않고 walking 구간의 time gap이 더 크게 나타난다는 문제도 남았다.
...
이 검증이 끝나야 3~4주차의 deskew 결과를 시간축 오류와 분리해 해석할 수 있다. Legged robot에서는 보행 중 body motion이 계속되므로, point time의 오차가 deskew를 거쳐 scan matching residual, map sharpness, odometry drift에까지 전달될 수 있다.
