...
4주차에는 실제 rosbag에서 한 scan 내부의 LiDAR pose 변화가 point cloud geometry를 얼마나 바꾸는지 봤다.

5주차의 질문은 그 다음 단계.

> 왜곡된 scan이 map에 붙을 때, registration residual과 inlier ratio가 어떻게 나빠지는가?

전체 연구 흐름으로 보면 이번 주 위치는 여기.
...
다만 이번 5주차 실험 데이터는 로봇개 bag이 아니라 UNIST Livox 데이터다.

이 선택의 이유는 단순하다. fixed-map registration residual을 보여주려면 map 구조가 선명하고 scan이 잘 보여야 한다. Go2 야외 bag은 연구 주제와 더 직접적으로 연결되지만, 블로그 대표 그림으로는 구조가 덜 선명했다. 그래서 5주차는 UNIST 데이터로 evaluator를 먼저 검증하고, quadruped gait-induced motion 주장은 이후 Go2 데이터에서 다시 연결하는 흐름으로 잡는다.

즉 5주차는 SLAM 전체를 새로 만드는 주가 아니다.

고정된 map에 현재 scan을 붙여 보고, scan distortion이 registration 품질 지표에 어떻게 나타나는지 측정하는 evaluator를 만드는 주.

### **1~4주차가 여기로 이어지는 방식**

5주차는 앞 주차 개념을 한 번에 다시 쓰는 지점.
...
즉 5주차의 residual은 갑자기 나온 metric이 아니다.

앞에서 공부한 frame, time, deskew, trajectory estimate가 모두 맞아야 registration residual도 의미 있게 해석된다.

## **1. Registration은 무엇을 하는가**

LiDAR SLAM에서 registration은 현재 scan을 이전 scan 또는 map에 맞추는 과정이다.
...
여기서 $R$은 rotation, $t$는 translation.

5주차에서는 full SLAM loop가 아니라 fixed map 기준의 scan-to-map registration 관점으로 본다.
...
이렇게 하면 map이 계속 변하는 효과를 줄이고, scan 자체의 왜곡이 registration metric에 어떻게 드러나는지 보기 쉽다.

### **Scan-to-Scan과 Scan-to-Map**

registration은 크게 두 방식으로 볼 수 있다.
...
scan-to-scan은 짧은 시간 간격에서는 계산이 가볍고 drift를 바로 추정하기 좋다.

하지만 직전 scan도 이미 왜곡되어 있거나 pose가 틀려 있으면, 두 scan 사이의 오차가 어디서 왔는지 분리하기 어렵다.

scan-to-map은 더 강한 기준면을 둔다.
...
그래서 fixed map은 "정답"이라기보다, 모든 scan을 같은 기준으로 비교하기 위한 reference surface이다.

### **Registration은 residual 최소화 문제**

registration을 수식으로 보면 결국 residual을 줄이는 문제.
...
여기서 $r_i$는 각 point의 residual이고, $\rho(\cdot)$는 outlier 영향을 줄이기 위한 robust loss.

이 관점에서 보면 registration 품질은 다음 세 가지가 같이 결정한다.
...
이번 5주차 evaluator는 2번과 일부 1번을 먼저 계측한다.

아직 3번, 즉 pose update 자체를 반복해서 푸는 ICP/Gauss-Newton 단계는 넣지 않았다.

## **2. Point-to-Plane Residual**

이번 주 핵심 수식은 point-to-plane residual이다.

현재 scan의 한 point를 $\mathbf{p}_i$라고 하겠다.
...
map에서 이 point와 대응되는 local plane의 대표점을 $\mathbf{q}_i$, normal vector를 $\mathbf{n}_i$라고 하면 point-to-plane residual은 다음.
...
여기서 residual은 signed distance.

하지만 metric으로 볼 때는 보통 절댓값을 본다.
...
point-to-plane residual은 점과 점 사이의 거리를 직접 보는 것이 아니라, map 표면의 normal 방향으로 얼마나 떠 있는지를 본다.
...
벽면을 예로 들면, 같은 벽 위에서 조금 옆으로 밀린 point는 point-to-point 거리로는 크게 보일 수 있다.

하지만 벽의 normal 방향으로는 거의 벗어나지 않았을 수 있다.

SLAM registration에서 더 중요한 것은 보통 "표면에서 떴는가".

그래서 벽, 바닥, 기둥처럼 표면 구조가 많은 LiDAR scene에서는 point-to-plane residual이 point-to-point residual보다 pose 보정 방향을 더 잘 준다.

단점도 있다.

한 평면만 보면 그 평면을 따라 미끄러지는 방향은 residual이 거의 변하지 않는다.
...
그래서 실제 registration은 여러 방향의 벽, 바닥, 모서리, 기둥이 같이 있어야 6DoF pose가 잘 구속된다.

## **3. Correspondence와 Plane Fitting**

point-to-plane residual을 계산하려면 먼저 각 scan point에 대응되는 map surface를 찾아야 한다.
...
plane normal은 local map point들의 covariance를 보고 구할 수 있다.

작은 eigenvalue에 대응되는 eigenvector가 local plane의 normal 방향.

이 부분이 중요한 이유는, residual이 단순히 두 point 사이의 거리만 보는 것이 아니기 때문이다.

벽이나 바닥 같은 구조물에서는 point-to-plane residual이 point-to-point distance보다 registration 방향을 더 잘 설명한다.

local plane fitting은 대략 다음 계산.
...
이 covariance의 eigenvalue를 보면 local geometry를 알 수 있다.
...
normal 방향의 부호는 중요하지 않다.

$\mathbf{n}_i$ 대신 $-\mathbf{n}_i$를 써도 signed residual의 부호만 바뀌고, 절댓값 residual은 같다.

하지만 normal 자체가 불안정하면 residual도 흔들린다.

따라서 sparse point, glass처럼 반사가 이상한 surface, dynamic object, map edge 근처 point에서는 correspondence와 normal을 그대로 믿으면 안 된다.

## **4. Inlier Ratio와 Residual Metric**

모든 correspondence를 다 믿으면 안 된다.

scan point가 map 범위 밖에 있거나, 잘못된 plane에 붙거나, residual이 너무 크면 outlier로 봐야 한다.

이번 구현에서는 크게 두 threshold를 사용했다.
...
여기서 `residual_p95`는 특히 유용하다.

평균은 일부 쉬운 point가 많으면 좋아 보일 수 있지만, p95는 scan에서 크게 어긋난 tail 쪽을 더 잘 보여준다.

이번 UNIST evaluator에서는 다음 threshold를 사용했다.
...
여기서 `correspondence_count`와 `inlier_count`는 다르다.
...
robust cost는 큰 residual을 그대로 제곱해서 벌주지 않고, 일정 이상에서는 영향을 줄인 cost.

이 값은 outlier에 덜 민감하지만, 반대로 p95처럼 tail 악화를 직관적으로 보여주지는 않는다.

그래서 이번 글에서는 `residual_p95`와 `inlier_ratio`를 중심으로 해석한다.

## **5. 나빠지는 방식은 하나가 아니다**

이번 주에서 가장 조심해야 할 부분은 registration degradation을 한 가지 현상으로 뭉뚱그리지 않는 것.

왜곡된 scan이 map에 잘 안 붙는 경우는 최소 세 가지로 나눠 볼 수 있다.
...
그래서 registration을 볼 때는 residual 하나만 보면 부족하다.

residual, inlier ratio, nearest-neighbor distance, pose update 안정성을 같이 봐야 한다.

### **Initial Pose가 중요한 이유**

registration은 보통 local optimization이다.

처음 pose가 어느 정도 맞아 있어야 nearest neighbor도 맞고, plane normal도 맞고, residual도 의미가 있다.

초기 pose가 많이 틀리면 문제는 이렇게 바뀐다.
...
이번 실험에서 `initial_pose = identity`로 고정한 이유도 여기에 있다.

처음부터 pose optimization까지 섞으면, residual 변화가 scan 왜곡 때문인지, optimizer가 다른 local minimum으로 간 것인지 구분하기 어려워진다.

### **Gauss-Newton과 Hessian은 다음 단계**

full point-to-plane ICP에서는 보통 pose update $\delta \boldsymbol{\xi}$를 반복해서 푼다.
...
전체 residual을 쌓으면 normal equation은 다음 형태가 된다.
...
Hessian이 잘 conditioned 되어 있으면 pose update 방향이 안정적.

반대로 구조가 부족하면 특정 방향이 거의 관측되지 않는다.

예를 들어 긴 복도에서는 앞뒤 translation이나 yaw가 헷갈릴 수 있고, 큰 평면 하나만 있으면 평면을 따라 움직이는 방향이 약하게 구속된다.

그래서 5주차에서 말하는 `geometry degeneracy`는 단순히 residual이 크다는 뜻이 아니다.
...
이것이 다음 주에 Hessian condition과 initial pose perturbation sweep을 보려는 이유.

## **6. 최소 구현 구조**

5주차 evaluator의 최소 코드는 다음 구조다.
...
이 코드는 pose optimization을 끝까지 구현한 ICP가 아니다.

우선 fixed pose에서 scan이 map에 얼마나 잘 붙는지 측정하는 residual evaluator.

## **7. UNIST Fixed-Map Registration 구현**

이번 주에는 계획에서 끝내지 않고 UNIST 데이터로 최소 구현까지 만들었다.
...
즉 이 결과는 full SLAM 결과가 아니다.

또한 map이 ground truth라는 뜻도 아니다.

다만 같은 fixed map 후보를 기준으로 raw scan, gyro-deskewed scan, synthetic distorted scan의 registration metric을 비교할 수 있다.

## **8. 대표 3D 그림이 의미하는 것**

5주차 대표 그림은 VTK로 다시 렌더링한 `vtk_low_oblique_residual_triptych.png`.
...
이 그림은 세 개의 scan을 fixed map 위에 올려 놓고 비교한 것.
...
색이 입혀진 점들은 평가 대상 scan point.

색은 point-to-plane residual의 절댓값을 의미한다.
...
따라서 이 그림은 "deskew가 예쁘게 됐다"를 직접 증명하는 그림이 아니다.

정확히는 다음을 보여주는 그림.

> 같은 fixed map 기준으로 볼 때, 각 scan point가 local plane에서 얼마나 떨어져 있는가?

그래서 5주차 글에서는 이 그림을 registration residual 분포를 보는 진단 그림으로 해석해야 한다.

처음 matplotlib 3D 그림만 보면 데이터가 2D처럼 보일 수 있다. 하지만 데이터 자체는 2D가 아니다.
...
2D처럼 보였던 이유는 point cloud가 빈 공간을 채우는 volume data가 아니라, 벽, 바닥, 구조물 표면을 샘플링한 surface data이기 때문이다. 즉 LiDAR point cloud는 3D 좌표를 갖지만, 시각적으로는 얇은 표면들의 집합처럼 보이는 것이 자연스럽다.

보조로 축과 colorbar가 있는 matplotlib 3D 그림도 남겨 둔다. 수치 비교를 읽기에는 이 그림이 더 직접적.
...
가장 중요한 비교는 raw와 synthetic strong이다.
...
strong synthetic yaw distortion은 raw보다 residual p95가 커지고, inlier ratio가 낮아졌다.

즉 controlled distortion이 fixed-map registration metric을 악화시키는 방향은 확인됐다.

이 표에서 읽어야 하는 포인트는 세 가지.

첫째, raw와 gyro-deskewed의 차이는 매우 작다.
...
rotation-only gyro deskew가 p95 residual을 약간 줄였지만, inlier ratio는 오히려 raw보다 낮다.

이것은 이상한 결과라기보다, 현재 evaluator의 한계를 보여주는 결과.
...
즉 gyro-deskewed가 모든 metric에서 이겨야 한다고 기대하면 안 된다.

둘째, synthetic strong은 raw보다 더 나쁜 방향으로 움직였다.
...
같은 fixed map, 같은 evaluator에서 강한 artificial yaw distortion을 넣었을 때 inlier ratio가 떨어졌다.

이 부분이 5주차의 핵심 관찰이다.

셋째, residual p95와 inlier ratio는 서로 보완적.

residual p95는 살아남은 correspondence들 중 tail이 얼마나 나쁜지 보여준다.

inlier ratio는 threshold를 통과한 point가 얼마나 남았는지 보여준다.

따라서 좋은 registration은 보통 다음 형태를 기대한다.
...
이번 구현은 이 중 pose update 안정성까지는 아직 보지 않았다.

## **10. 조심해야 하는 해석**

이번 결과에서 가장 조심할 점은 다음.
...
특히 gyro-deskewed scan은 raw보다 p95 residual은 약간 작지만, inlier ratio와 robust cost는 더 좋아졌다고 말하기 어렵다.

따라서 이번 결과를 이렇게 쓰면 안 된다.
...
또 하나 중요한 점은 synthetic distortion sweep이 완전히 단조적이지 않았다는 것.

weak, medium, strong 순서로 residual p95와 robust cost가 항상 증가하지는 않았다.

그래서 "distortion이 강해질수록 모든 metric이 단조적으로 나빠졌다"라고 쓰면 안 된다.

현재 말할 수 있는 결론은 더 제한적.
...
다음 주에 더 강한 결과로 가려면 fixed-pose residual evaluator에서 pose optimization으로 넘어가야 한다.
...
이렇게 해야 registration metric 변화가 단순 residual 진단을 넘어서 실제 pose estimation 안정성까지 이어지는지 볼 수 있다.
...
Residual이 커졌다는 수치만으로는 원인을 알 수 없다. Correspondence가 깨진 것인지, 같은 plane 위에서 point 위치만 흔들린 것인지, geometry가 pose update를 충분히 구속하지 못한 것인지 구분해야 한다.

이번 UNIST 결과는 이 구분을 시작하기 위한 fixed-pose evaluator이다. 다음 단계에서 point-to-plane Jacobian과 Gauss-Newton update를 넣어야 metric 변화가 실제 pose estimation의 convergence와 conditioning으로 이어지는지 확인할 수 있다. 현재 결과는 4주차 deskew 시각화와 이후 registration degradation 분석을 잇는 수준으로 해석한다.
