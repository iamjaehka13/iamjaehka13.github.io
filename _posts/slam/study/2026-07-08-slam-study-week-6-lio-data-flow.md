...
6주차는 LIO를 수식으로 새로 구현하는 주가 아니다.

이미 있는 LIO 코드 안에서 데이터가 어떻게 흘러가는지 추적하는 주.
...
6주차는 이 개념들을 실제 LIO 시스템 안에서 연결한다.
...
따라서 6주차의 목표는 이 경로가 LIO 코드 안에서 어디에 있는지 찾는 것이다.
...
LIO는 LiDAR-Inertial Odometry.

말 그대로 LiDAR와 IMU를 같이 사용해서 로봇의 pose를 추정한다.
...
하지만 IMU만 적분하면 bias 때문에 drift가 누적된다.

LiDAR는 느리지만 주변 geometry를 본다.

LiDAR scan을 map에 맞추면 pose correction을 할 수 있다.
...
즉 LiDAR는 IMU propagation의 drift를 geometry로 보정한다.

LIO 전체 흐름은 다음처럼 볼 수 있다.
...
LIO가 추정하는 state는 보통 다음 항목을 포함한다.
...
6주차에서 state propagation 수식을 모두 외우는 것이 목적은 아니다.
...
gyro raw measurement는 다음처럼 생각할 수 있다.
...
accelerometer는 gyro보다 더 헷갈린다.

gravity와 frame 변환이 같이 들어가기 때문.
...
IMU accelerometer는 단순히 로봇이 움직이는 가속도만 보는 것이 아니다.

로봇이 가만히 있어도 gravity 때문에 값이 나온다.

gravity를 제대로 처리하지 못하면, 로봇이 가만히 있는데도 계속 가속한다고 착각할 수 있다.

그래서 IMU propagation에서 gravity는 position과 velocity 예측에 직접 들어간다.

bias도 중요하다.

예를 들어 gyro가 실제로는 회전이 없는데도 계속 다음 값을 낸다고 하겠다.

```text
gyro bias = 0.01 rad/s
```

이 작은 값도 계속 적분되면 orientation drift가 된다.

accelerometer bias는 velocity와 position drift로 이어진다.
...
그래서 LIO state에는 보통 gyro bias와 accelerometer bias가 들어간다.

LiDAR update는 pose뿐만 아니라 bias 추정에도 영향을 줄 수 있다.

이 점이 LiDAR와 IMU를 단순히 따로 처리하는 방식과 LIO가 다른 부분.

## **5. IMU Propagation**

IMU propagation은 IMU data를 적분해서 pose를 예측하는 과정이다.
...
IMU propagation은 6주차에서 특히 중요하다.
...
즉 IMU prediction이 틀리면 다음 일이 이어진다.
...
그래서 LIO 코드를 볼 때 IMU sample 경로를 point 경로와 별도로 추적해야 한다.

## **6. LiDAR Measurement Update**

LiDAR update는 5주차 registration과 연결된다.

5주차에서 본 point-to-plane residual은 다음이었다.
...
그래서 6주차 logging에서는 prediction pose와 update pose를 분리해서 봐야 한다.
...
loosely coupled는 LiDAR odometry 결과를 하나의 measurement처럼 사용한다.
...
쉽게 말하면 LiDAR가 먼저 pose를 만들고, 그 pose를 IMU 쪽과 나중에 합친다.

대표적으로 다음 구조가 여기에 가깝다.
...
tightly coupled는 LiDAR residual 또는 factor가 state update에 직접 들어간다.
...
FAST-LIO와 FAST-LIO2는 filter 기반 tightly coupled 예시로 이해하면 된다.

특히 FAST-LIO2는 feature extraction 없이 raw point를 map에 직접 등록하는 방향이라, point-to-map residual 경로를 추적하기 좋다.
...
LIO-SAM은 factor graph 기반 tightly coupled 예시로 볼 수 있다.
...
내 연구에서는 FAST-LIO2 계열이 특히 보기 좋다.

보고 싶은 값들이 내부 경로에 직접 있기 때문.
...
또 하나의 구분은 filter와 smoothing.

filter는 현재 state를 순차적으로 업데이트한다.
...
이전 모든 state를 계속 다시 최적화하지는 않는다.

장점은 빠르고 online 처리에 적합하다는 점이다.

smoothing 또는 factor graph는 여러 시점의 state를 그래프로 묶어서 최적화한다.
...
여러 state를 함께 조정할 수 있으므로, 과거 pose까지 다시 보정할 수 있다.
...
LiDAR update가 끝나면 보정된 pose가 나온다.

그다음 deskewed point를 map frame으로 변환해서 local map에 넣는다.
...
local map은 다음 scan의 correspondence 검색에 사용된다.

즉 map update는 다음 scan residual에 직접 영향을 준다.
...
그래서 logging할 때는 map insertion 전후도 봐야 한다.

loop closure는 이미 지나갔던 장소를 다시 인식해서 누적 drift를 고치는 backend 기능.

하지만 6주차에서 보고 싶은 것은 frontend의 scan-level 동작이다.

그래서 odometry를 재현할 때 loop closure가 있으면 꺼두는 편이 해석이 쉽다.
...
IMU propagation은 두 가지 역할을 동시에 한다.
...
따라서 IMU propagation이 틀어지면 deskew와 registration이 동시에 영향을 받는다.

## **12. Logging 추가 위치**

6주차에서 코드를 깊게 고치는 것이 목적은 아니다.
...
이 값은 IMU propagation으로 LiDAR update 전에 예측한 scan pose.
...
이 값은 LiDAR residual이 IMU prediction을 얼마나 고쳤는지 보여준다.

update가 크면 다음 가능성이 있다.
...
filter 계열에서는 covariance가 state uncertainty를 나타낸다.
...
저장이 어렵다면 최소한 update magnitude를 저장한다.
...
이 값은 실시간 처리 여유와 병목을 볼 때 필요하다.

## **13. 저장할 CSV 구조**

처음에는 scan 단위 CSV면 충분하다.
...
이렇게 해야 bag 하나를 돌렸을 때 log가 감당 가능한 크기로 남는다.

## **14. 코드 추적 키워드와 노트 템플릿**

코드에서 point 경로를 찾을 때 검색할 키워드는 다음이다.
...
6주차는 구현보다 코드 독해와 logging 설계가 중요하다.
...
위에서 정리한 logging 위치를 실제 FAST-LIO2 ROS2 코드에 붙여 봤다.

사용한 데이터는 4~5주차에서 계속 사용한 UNIST Livox bag.
...
대표 그림은 scan 단위로 pose update, residual, processing time을 함께 본 것이다.
...
이 그림에서 중요한 점은 `pose update norm`이 LiDAR update가 IMU prediction을 얼마나 보정했는지를 보여준다는 것.
...
처음에는 20초 smoke run으로 실행 여부만 확인했다.

블로그용 시각화는 너무 짧은 구간보다 full bag이 맞기 때문에, 이후 전체 bag을 다시 replay했다.

full bag trace의 요약은 다음.
...
전체 odometry trace의 span은 다음 정도였다.
...
다음 그림은 residual tail과 update magnitude를 직접 비교한 것이다.
...
이 그림은 residual이 커졌을 때 update도 항상 같이 커진다고 단순하게 말하기 어렵다는 점을 보여준다.

즉 residual, effective feature count, map geometry, initial prediction이 같이 작용한다. 그래서 이후 실험에서는 residual 하나만 보지 말고 pose update norm과 feature count도 같이 봐야 한다.

마지막으로 FAST-LIO2가 낸 odometry trace를 residual p95 색으로 칠했다.
...
정적 그림만으로는 흐름이 잘 안 보이기 때문에, full bag 기준 animation 후보도 같이 만들었다.

GIF로도 뽑았지만 본문에는 용량이 작은 animated WebP를 넣었다.
...
제일 괜찮은 것은 아래 full-bag registered cloud orbit.

이 animation은 `/cloud_registered`와 `/Odometry`를 직접 수집해서 만들었다.

즉, FAST-LIO2가 full bag replay 중 publish한 registered cloud와 path를 같이 보여주는 그림.
...
다음은 full bag 전체 trajectory를 시간에 따라 누적한 animation이다.

이 그림은 path가 시간에 따라 어떻게 쌓이는지 보기 좋다. residual p95를 같이 붙였기 때문에, 단순 경로 그림보다 LIO 내부 update trace와 연결해서 볼 수 있다.
...
마지막으로, full bag에서 30초 window를 sliding하면서 보기 좋은 구간을 자동으로 골랐다.

선정 점수는 path length, x/y/z span, residual p95 range, pose update 크기를 섞어서 계산했다.
...
아래는 1등 구간인 70-100초 local map build animation.
...
이 trajectory 그림은 보기에는 odometry 결과처럼 보이지만, 아직 평가 결과로 쓰면 안 된다.
...
이번 결과의 의미는 성능 검증이 아니라, 6주차 목표였던 LIO 내부 변수 추적을 실제 코드에서 시작했다는 것.

## **18. 이번 trace로 말할 수 있는 범위**

6주차 글에서 조심할 점은 명확하다.

이번 주는 특정 LIO 알고리즘의 성능을 검증한 것이 아니다.

또 새로운 odometry 결과를 만든 것도 아니다.
...
아직은 그런 claim을 할 단계가 아니다.

이번 추가 실험에서는 실제 FAST-LIO2 코드에 logging을 넣고, 같은 bag에서 scan별 residual, pose update, map insertion 관련 값을 뽑았다.

다만 아직 calibration sensitivity, time offset sweep, ground-truth trajectory 비교는 하지 않았다.

## **19. 알고리즘 이름보다 코드 위치를 남기기**

> LIO 안에서 IMU가 pose를 예측하고, LiDAR point residual이 그 pose를 보정하며, 보정된 point가 map에 들어가는 전체 경로를 추적하는 주다.

이 단계에서는 알고리즘 이름보다 아래 연산이 실제 코드의 어디에서 일어나는지 찾는 편이 중요하다.
...
이 위치를 알아야 7주차부터 logging을 추가하고, 같은 scan에서 prediction, deskew, residual, update, map insertion을 연결할 수 있다. 내 연구에서는 아래 경로를 실제 변수로 기록하는 것이 다음 목표.
...
이번 FAST-LIO2 trace는 이 경로를 기록하기 시작한 단계이며, calibration sensitivity와 time offset, ground-truth trajectory 비교는 아직 남아 있다.
