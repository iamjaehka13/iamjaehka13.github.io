...
DQN(Deep Q-Network)은 Naive DQN의 불안정성을 줄이기 위해 두 장치를 도입했습니다.

- **경험 리플레이(Experience Replay)**
- **타겟 네트워크(Target Network)**

두 장치는 학습 데이터와 TD target이 동시에 크게 흔들리지 않도록 역할을 나눕니다.
...
Double Q-Learning은 **행동 선택**과 **가치 평가**를 서로 다른 추정치에 맡깁니다.
...
환경의 동역학 $p(s_{t+1} \mid s_t, a_t)$를 직접 알 필요는 없습니다. 에이전트가 궤적을 샘플링하고, 그 궤적의 보상과 로그 정책 확률의 기울기로 정책을 업데이트합니다.
...
A3C는 비동기 worker로 다양한 경험을 빠르게 모으고, A2C는 worker의 결과를 동기화해 업데이트 일관성과 GPU 효율을 높입니다.

## **11. 가치 기반 방법에서 정책 최적화로**

DQN 계열의 발전은 함수 근사로 생긴 불안정성, 과대평가, 경험의 중요도 차이를 하나씩 다루는 과정입니다. Policy Gradient 계열은 행동 가치를 거쳐 선택하는 대신 정책을 직접 최적화하고, Actor-Critic은 두 접근을 결합해 기울기의 분산을 줄입니다.

연속적인 로봇 제어에서는 정책을 얼마나 크게 업데이트할지와 연속 행동을 어떻게 출력할지가 특히 중요합니다. 다음 글에서는 이 문제를 다루는 **DDPG, TRPO, Natural Policy Gradient, PPO**의 관계를 이어서 봅니다.
