...
[2편](/posts/deep-learning-2-gradient-descent/)에서는 신경망이 cost를 줄이기 위해 negative gradient 방향으로 parameter를 수정한다는 것을 정리했다.

$$
\theta \leftarrow \theta-\eta\nabla C(\theta)
$$

그런데 이 식은 중요한 계산 하나를 이미 알고 있다고 가정한다.
...
신경망에는 수많은 weight와 bias가 있다. 각 parameter가 cost에 미치는 영향을 어떻게 전부 계산할 수 있을까?

이 글은 3Blue1Brown의 **Deep Learning Chapter 3: Backpropagation, intuitively**와 **Chapter 4: Backpropagation calculus**를 함께 보며 다음 질문에 답한다.
...
backpropagation은 학습 전체를 뜻하지 않는다. 학습 한 step 안에서 미분값을 효율적으로 구하는 알고리즘.
...
출력 layer의 activation을 $\mathbf{a}^{(L)}$라고 할 때 한 sample의 squared cost:
...
정답이 숫자 2라면 2번 output neuron은 1에 가까워져야 하고, 나머지 output neuron은 0에 가까워져야 한다.
...
따라서 출력에서 시작하면 각 activation을 어느 방향으로 움직여야 cost가 줄어드는지 알 수 있다. 이 원하는 변화가 역방향 계산의 출발점.
...
이다. 이 output activation을 바꾸는 방법은 세 가지.
...
세 번째 방법이 중요하다. 이전 activation은 그 앞 layer의 weight와 bias로 결정되므로, 같은 질문을 한 layer 앞에서 다시 할 수 있다.
...
이전 neuron이 강하게 활성화되어 있다면 연결 weight의 작은 변화가 $z_j^{(L)}$를 크게 바꾼다. 반대로 activation이 거의 0이면 그 weight를 바꿔도 영향이 작다.

따라서 직관적으로는 다음과 같이 읽을 수 있다.

> 활성화된 neuron에서 나온 연결일수록 현재 sample의 학습 신호를 더 크게 받는다.

하지만 이것만으로 weight를 무조건 키운다는 뜻은 아니다. 변화의 부호와 전체 크기는 출력 오차와 activation function의 derivative까지 함께 곱해져 결정된다.
...
즉 output neuron의 오차 신호는 연결 weight에 비례해 이전 neuron으로 전달된다. 하나의 이전 neuron이 여러 output neuron에 연결되어 있다면, 모든 경로에서 온 영향을 더해야 한다.
...
이렇게 마지막 layer에서 얻은 정보를 한 layer씩 뒤로 전달하기 때문에 **backpropagation**이라고 부른다.

## **6. 역방향으로 같은 문제를 반복한다**

이전 layer의 activation을 어느 방향으로 바꿔야 하는지 알았다면, 그 activation을 만든 weight와 bias가 받을 변화도 계산할 수 있다.
...
각 layer마다 완전히 새로운 계산을 만드는 것이 아니다. 뒤에서 받은 오차와 현재 layer가 저장해 둔 값을 이용해 같은 형태의 계산을 반복한다.

## **7. 계산 그래프로 보면 더 명확하다**

수식을 단순하게 보기 위해 각 layer에 neuron이 하나씩만 있는 network를 생각한다.
...
forward pass는 이 화살표를 왼쪽에서 오른쪽으로 따라 값을 계산한다. backward pass는 오른쪽에서 왼쪽으로 따라가며 sensitivity를 계산한다.

## **8. Chain rule로 경로의 영향을 곱한다**

$w^{(L)}$가 바뀌면 $z^{(L)}$가 바뀌고, 그 결과 $a^{(L)}$가 바뀌며, 마지막으로 $C_0$가 바뀐다.
...
마지막 식이 바로 앞 layer로 전달되는 신호. 이제 이 값을 시작점으로 다시 chain rule을 적용할 수 있다.

## **10. Error signal을 정의한다**

실제 network 식을 간결하게 쓰기 위해 layer $l$의 pre-activation에 대한 cost의 derivative를 error signal로 정의한다.
...
가 된다. $\odot$는 element-wise product이다.

여기서 $\boldsymbol{\delta}$는 단순히 예측값에서 정답을 뺀 값이 아니다. activation function의 local derivative까지 포함한 **pre-activation에 대한 cost의 변화율**.
...
transpose가 등장하는 이유는 forward pass의 선형변환 방향을 거꾸로 따라가며 각 이전 neuron에 연결된 영향을 합쳐야 하기 때문.

## **12. Weight와 bias의 gradient를 얻는다**

layer $l$의 error signal을 구하면 parameter gradient는 바로 계산된다.
...
즉 weight gradient는 다음 두 값의 곱.

- 이 연결이 도착하는 neuron의 error signal $\delta_j^{(l)}$
- 이 연결이 출발하는 neuron의 activation $a_k^{(l-1)}$

Chapter 3에서 본 “활성화된 연결이 더 큰 변화를 받는다”는 직관이 이 식에 그대로 나타난다.

## **13. Forward pass의 값을 저장하는 이유**

backward pass 식에는 $\mathbf{a}^{(l-1)}$, $\mathbf{z}^{(l)}$, $W^{(l)}$가 필요하다.

그래서 일반적인 학습 과정은 forward pass에서 중간값을 저장해 두고 backward pass에서 재사용한다.
...
중간값을 저장하면 계산을 줄일 수 있지만 memory를 사용한다. 깊은 network에서 activation memory가 큰 이유도 여기에 있다.

## **14. 왜 각 parameter를 따로 미분하지 않는가**

parameter마다 처음부터 forward 계산을 다시 따라가며 derivative를 구하면 같은 중간 계산이 반복된다.
...
이 공유가 핵심. backpropagation은 chain rule 자체가 아니라, 계산 그래프의 공통 부분을 재사용하며 chain rule을 적용하는 효율적인 방법이다.

## **15. 다음 layer에서 오는 영향을 모두 더한다**

layer $l$의 $k$번째 neuron은 다음 layer의 여러 neuron과 연결된다. 따라서 $a_k^{(l)}$이 cost에 미치는 영향은 다음 layer의 각 연결을 통해 전달되는 영향을 모두 더해 구해야 한다.
...
즉 다음 layer의 각 error signal에 해당 연결 weight를 곱하고, 그 결과를 $j=1$부터 $n_{l+1}$까지 모두 더한다.
...
. 여기서 activation function의 derivative까지 element-wise로 곱하면 현재 layer의 error signal을 얻는다.
...
따라서 matrix multiplication은 단순한 표기 축약이 아니라, 다음 layer의 여러 경로에서 오는 영향을 현재 layer의 각 neuron별로 모으는 계산이다.

## **16. Mini-batch에서는 gradient를 평균낸다**

지금까지는 한 sample의 cost $C_0$를 미분했다. 실제 학습에서는 mini-batch $B$에 있는 sample별 gradient를 평균낸다.
...
그다음 gradient descent가 이 값을 사용한다.
...
sample 하나가 원하는 변화는 다른 sample이 원하는 변화와 충돌할 수 있다. mini-batch 평균은 현재 batch 전체에서 cost를 줄이는 타협된 방향을 만든다.

## **17. 전체 알고리즘**

한 mini-batch에 대한 학습 step:
...
backpropagation을 외울 때는 식보다 정보의 흐름을 먼저 잡는 것이 좋다.
...
다음 질문에 답할 수 있으면 이번 글의 흐름을 이해한 것.
...
이제 신경망이 parameter를 학습하는 기본 흐름이 완성됐다. forward pass가 예측을 만들고, backpropagation이 gradient를 계산하며, gradient descent가 parameter를 수정한다.

다음 [4편](/posts/deep-learning-4-transformer/)에서는 이 기본 계산 구조가 Transformer에서는 어떤 순서로 확장되는지 정리한다.
