...
[5편](/posts/deep-learning-5-attention/)에서는 attention이 문맥 속 token 사이에서 정보를 이동시키는 계산을 살펴봤다.

Transformer block의 다른 큰 구성 요소는 MLP.
...
3Blue1Brown의 **Deep Learning Chapter 7: How might LLMs store facts**를 따라 아래 질문을 살펴본다.

> MLP는 hidden vector에서 어떤 feature를 감지하고, 학습된 정보를 다시 hidden space에 어떻게 써 넣을 수 있는가?

제목의 “어떻게 표현되는가”는 완전히 해결된 결론을 뜻하지 않는다. 실제 LLM의 지식 저장에 대한 완전한 mechanistic explanation은 아직 어려운 문제. 이 글은 영상의 toy example과 superposition 가설을 이용해 가능한 계산 구조를 이해하는 데 목적이 있다.
...
Model이 `basketball`에 높은 확률을 준다면 Michael Jordan과 basketball의 연관성이 수많은 parameter 어딘가에 학습되었다고 볼 수 있다.
...
영상은 연구 결과를 바탕으로 사실과 연관된 계산이 MLP에 많이 나타날 수 있다는 관점을 소개한다. 다만 하나의 사실이 하나의 neuron이나 하나의 MLP에 온전히 저장된다는 뜻은 아니다.
...
Attention에서는 $T\times T$ attention pattern으로 token 사이의 정보를 섞었다. MLP에는 이런 sequence mixing이 없다. 대신 attention을 통해 이미 문맥을 흡수한 각 hidden vector를 feature 관점에서 읽고 수정한다.

## **3. MLP 전체 식과 shape**

MLP의 핵심은 두 번의 linear transformation과 그 사이의 nonlinear activation.
...
4편에서 고차원 hidden space의 방향이 의미나 feature와 연관될 수 있다고 봤다.

Toy example에서는 다음 세 방향이 있다고 가정한다.
...
Hidden vector $\mathbf{x}$가 특정 방향과 얼마나 정렬되는지는 dot product로 측정할 수 있다.

$$
s_m=\mathbf{x}\cdot\mathbf{m}
$$

이 해석은 이해를 위한 단순화. 실제 feature가 정확히 하나의 깨끗한 방향으로 존재한다고 먼저 가정해서는 안 된다.

## **5. Attention이 먼저 정보를 한 위치에 모을 수 있다**

`Michael Jordan`은 두 token에 걸쳐 있다. MLP는 token끼리 직접 통신하지 않으므로, Jordan 위치의 hidden vector가 두 이름을 모두 포함하려면 앞선 attention이 Michael 정보를 Jordan 위치로 전달해야 한다.
...
Row-vector convention에서 $W_{up}$의 각 열은 hidden space에서 하나의 probe direction으로 볼 수 있다.
...
$d_{ff}$개의 성분은 hidden vector에 대해 서로 다른 질문을 동시에 던지는 것처럼 해석할 수 있다.
...
아직 linear score만으로는 “둘 다 있는가”를 정확히 분리하지 못한다.
...
이 된다. Bias는 feature detector가 켜지는 threshold를 조절하는 역할을 할 수 있다.
...
Toy example에서는 Michael과 Jordan이 함께 있을 때만 $a_1>0$이므로 AND gate처럼 동작한다.

실제 Transformer에서는 ReLU뿐 아니라 GELU, SwiGLU 같은 activation과 gated MLP가 사용되기도 한다. 구체적인 식은 architecture마다 다르지만 linear transformation 사이에 nonlinearity를 넣는 핵심 역할은 같다.

## **10. Transformer에서 neuron은 무엇인가**

이 문맥에서 neuron은 activation function을 통과한 중간 vector의 한 성분이다.
...
Neuron 하나는 다음 두 parameter 방향과 연결된다.
...
이고 $a_1=1$일 때 basketball 방향이 update에 포함된다.

하나의 output direction은 basketball뿐 아니라 Chicago Bulls, number 23처럼 함께 유용한 여러 feature 성분을 포함할 수도 있다.

## **12. Residual connection으로 지식을 써 넣는다**

MLP가 만든 update는 기존 vector에 더해진다.
...
입력 vector가 Michael과 Jordan을 모두 포함하면 basketball 방향을 더하고, 그렇지 않으면 이 toy neuron은 아무 변화도 만들지 않는다.
...
Bias는 각 행에 broadcasting된다. 같은 $W_{up}$과 $W_{down}$이 모든 position에 적용되지만, 각 position의 hidden vector가 다르므로 neuron activation도 다르다.
...
영상의 GPT-3 숫자에서는 MLP parameter가 전체 parameter의 큰 비중을 차지한다. 이는 MLP가 model capacity에서 주변적인 부분이 아니라는 뜻이다.

## **15. Toy example을 실제 model의 결론으로 착각하면 안 된다**

지금까지는 설명을 위해 다음처럼 매우 깨끗한 구조를 가정했다.
...
수학적으로 이런 계산이 가능하다는 것은 맞는다. 하지만 실제 model에서 하나의 neuron이 하나의 사람이나 사실만 표현한다는 결론은 아니다.

실제 representation은 여러 neuron과 layer에 분산될 수 있으며, 한 neuron이 서로 관련 없어 보이는 여러 feature에 반응하는 polysemantic behavior를 보일 수도 있다.

## **16. Superposition 문제**

$n$차원 공간에서 서로 완전히 직교하는 방향은 최대 $n$개.
...
하지만 feature 방향이 완전히 직교할 필요 없이 거의 직교해도 된다면, 작은 interference를 허용하는 대신 차원 수보다 훨씬 많은 feature를 표현할 가능성이 생긴다.

이처럼 제한된 activation dimension 위에 더 많은 feature를 겹쳐 표현한다는 관점을 **superposition**이라고 한다.

## **17. 고차원에서는 거의 직교하는 방향이 많다**

고차원 random vector들은 서로 90도에 가까운 각도를 이루는 경향이 있다.
...
Normalized vector $\mathbf{f}_i$와 $\mathbf{f}_j$의 dot product가 작으면 거의 직교한다.

$$
\left|\mathbf{f}_i^T\mathbf{f}_j\right|\ll1
$$

완전히 0은 아니므로 feature 사이에 interference가 생길 수 있지만, model이 sparse하게 활성화되는 feature를 사용한다면 그 비용을 감수하고 더 많은 feature를 표현하는 것이 유리할 수 있다.

## **18. Feature와 neuron은 일대일이 아닐 수 있다**

Superposition이 사용된다면 하나의 feature는 특정 neuron 하나가 아니라 여러 neuron activation의 조합으로 나타난다.
...
Neuron basis와 feature basis가 일치할 수도 있지만 반드시 그런 것은 아니다.
...
Sparse autoencoder는 reconstruction error를 작게 만들면서 $\mathbf{s}$의 많은 성분이 0이 되도록 유도한다.
...
목표는 neuron 좌표에 겹쳐 있는 feature direction을 더 분리된 sparse representation으로 찾는 것. 다만 찾아낸 feature가 완전하거나 유일하다고 자동으로 보장되는 것은 아니다.

## **20. 지식은 고정된 데이터베이스 행이 아니다**

“MLP가 사실을 저장한다”는 표현을 database처럼 이해하면 안 된다.

```text
Michael Jordan → basketball
```

이라는 문자열 pair가 그대로 parameter 안에 기록되는 것이 아니라, 특정 contextual hidden state에 반응하는 detector와 그때 추가되는 output direction이 여러 weight에 분산되어 학습될 수 있다.

또한 최종 prediction에는 embedding, attention, 여러 MLP, residual stream, unembedding이 모두 관여한다. 특정 사실을 MLP 하나에만 독점적으로 귀속시키는 것은 과도한 단순화이다.
...
실제 network에서는 이 과정이 여러 layer에서 반복되며 더 복잡한 feature와 관계를 단계적으로 만들 수 있다.
...
단, 실제 feature는 개별 neuron과 일대일로 대응하지 않고 여러 neuron에 superposition되어 있을 수 있다.
...
이 글로 3Blue1Brown Deep Learning Chapter 1부터 7까지의 여섯 편 구성이 마무리된다. 신경망의 기본 계산에서 시작해 gradient descent와 backpropagation을 거쳐, Transformer의 전체 구조, attention, MLP와 feature representation까지 하나의 흐름으로 연결했다.
