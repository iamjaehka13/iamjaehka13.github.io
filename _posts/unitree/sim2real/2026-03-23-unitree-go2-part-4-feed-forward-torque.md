...
이전 글에서는 Go2가 command에 따라 base를 기울이기는 하지만, 실제로 발을 충분히 떼지 못하는 문제를 확인했습니다. 뒷다리 stiffness를 높였을 때 로봇이 움직이기 시작한 것을 보면, 단순한 reward 문제뿐 아니라 실제 actuator가 만들어내는 torque margin도 중요한 요인으로 보였습니다.
...
즉, policy가 출력하는 target position을 PD controller가 충분히 따라가지 못하고 있을 가능성이 있었습니다.
...
이 방식의 목적은 policy가 서 있기 위한 torque를 만들기 위해 target position을 과하게 틀지 않아도 되게 만드는 것입니다. 기본적으로 중력에 저항하는 torque가 들어가면, policy는 target position error를 이용해 몸무게를 억지로 버티는 대신 더 자연스러운 발 궤적을 출력할 수 있을 것으로 기대했습니다.
...
feed-forward torque는 real deploy에서 부족했던 torque margin을 확인하는 데 도움이 되었습니다. 하지만 최종 해결책이라고 보기는 어려웠습니다.

deploy에서만 추가 torque를 넣으면, training에서 policy가 본 actuator model과 실제 deploy actuator model이 달라집니다. 즉, 문제를 해결하는 동시에 새로운 Sim2Real gap을 만들 수 있습니다.
