...
일반화된 정책 반복(GPI)은 대부분의 강화학습 알고리즘을 **정책 평가(Policy Evaluation)**와 **정책 개선(Policy Improvement)**의 상호작용으로 설명합니다.
...
MC는 상태-행동 쌍의 가치를 실제 반환값($G_t$)들의 평균으로 추정합니다.
...
MC는 실제 반환값을 끝까지 관측하므로 편향은 없지만 분산이 크고, TD는 아직 정확하지 않은 추정값을 사용해 약간의 편향을 받아들이는 대신 분산을 낮춥니다. 이 선택이 이후 알고리즘의 학습 성격을 가릅니다.

## **6. Sarsa vs Q-Learning**

Sarsa와 Q-Learning은 **탐험으로 실제 선택한 행동의 위험을 가치 평가에 반영하는가**에서 갈립니다.
...
Double Q-Learning은 이 문제를 줄이기 위해 두 개의 Q함수, $Q_A$와 $Q_B$를 사용해 **행동 선택**과 **가치 평가**를 분리합니다.
...
MC, TD, Sarsa, Q-Learning은 모두 환경 모델 대신 표본으로 가치를 추정하지만, 반환값을 언제 만들고 어떤 정책의 다음 행동을 쓰는지에 따라 학습 특성이 달라집니다. Double Q-Learning까지 오면 추정 과정에 생기는 편향도 별도의 설계 대상이 됩니다.

지금까지는 각 상태와 행동의 값을 표에 저장할 수 있다고 가정했습니다. 이미지나 연속 센서 상태처럼 경우의 수가 커지면 이 가정이 깨지므로, 다음 글에서는 표를 신경망 함수 근사기로 바꾸는 **Deep Reinforcement Learning (DRL)**을 다룹니다.
